...
- **Logical timestamp in the ID prefix.** The 16-digit timestamp component of the sortable ID is a process-local logical ordering timestamp, not necessarily the message's stored timestamp. It is seeded from the first admitted timestamp and advances when the caller supplies a larger timestamp; it never decreases, so the lexical cursor order is always well-defined.
- **High-water advancement is rate-limited.** A single admitted timestamp can advance the logical high-water mark by at most `MAX_HIGH_WATER_ADVANCE_MS` (a few seconds). Far-future timestamps are still admitted but their contribution to the ordering key is capped, preventing a single future input from pinning the global sequence bucket for an extended interval. The initial seed is also clamped to wall-clock + `MAX_HIGH_WATER_ADVANCE_MS` so a far-future first call cannot pin normal writes.
