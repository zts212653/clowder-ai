...
> 参与者：Ragdoll（方案设计）、Maine Coon（reviewer 视角 + 硬规则）、operator（发起人）、GPT Pro（外部评审）

## 背景

2026-02-13 operator发现 `packages/api/src/domains/cats/services/` 堆积 70 个 .ts 文件，6 种不同职责混在一个扁平目录里。`docs/` 也有 270 个文件，从未系统归档。
