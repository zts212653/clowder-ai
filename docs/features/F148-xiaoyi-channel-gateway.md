...
3. **Gateway 集成**
   - Principal Link: `connectorId=xiaoyi`, `externalChatId=${agentId}:${params.sessionId}`（带 agentId 命名空间，防 P1 多账号键冲突）, `externalSenderId=owner:{agentId}`
...
| 多账号支持 | 同一 Cat Cafe 实例连接多个小艺智能体 (`Map<agentId, Connection>`) |
...
| 无用户级标识（OpenClaw 固有限制） | P0 用 `owner:{agentId}` 做 senderId，归属 connector 配置者；P1 多Agents模式补齐身份 |
...
| 5 | 多账号移至 P1 | P0 单账号足够 MVP，减少初始复杂度 | 2026-04-01 |
| 6 | adapter 内置 `sessionId→taskId` 状态机 | 流式出站必须带 taskId 回包；现有 `IStreamableOutboundAdapter` 接口不携带此上下文，由 adapter 内部 Map 管理（缅因猫 review P1-1） | 2026-04-01 |
| 7 | `externalChatId` 带 `agentId:` 命名空间 | 防 P1 多账号时不同 agent 的 sessionId 碰撞（缅因猫 review P2-1） | 2026-04-01 |
