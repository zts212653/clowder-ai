...
3. **Gateway 集成**
   - Principal Link: `connectorId=xiaoyi`, `externalChatId=params.sessionId`, `externalSenderId=owner:{agentId}`
...
4. **热加载**
   - .env 或 cat-config 中添加 ak/sk/agentId 后自动检测并连接
   - 运行时添加/移除账号无需重启 Cat Cafe
...
- [ ] AC-A6: Principal Link 正确建立 — `externalSenderId=owner:{agentId}` 归属 connector 配置者
- [ ] AC-A7: Session Binding — `params.sessionId` 映射 thread；`/new` `/threads` `/use` 正常工作
- [ ] AC-A8: 热加载 — .env/cat-config 添加凭证后自动连接，无需重启
