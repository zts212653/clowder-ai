...
created: 2026-04-19
revised: 2026-05-11
tips_exempt: "Renewed 2026-09-04 for the current F308 public-link projection only; no memory capability or user action changed."
---

# F169: Agent Memory Reflex — 愿景文档（vision artifact）

> **Status**: done（vision realized；B+C 已通过 F148/F163 实现；A operator拍板关闭，不做持久 Compiled Wiki）
...
> |----------|--------|---------|------|------|
> | Phase A | Compiled Wiki Self-Authoring | 关闭 | ✅ **operator拍板关闭**（2026-04-25） | 不做持久 compiled wiki；若痛点复现，方向是 query-time Feature Lens（现场投影，不存文件） |
...
>
> 本文档作为愿景研究产物保留：三层方向性主张 + 跨族视角论证 + ADHD 同构假设，已被两条主线（F148/F163）实现。Phase A operator 2026-04-25 拍板关闭——不做持久文档，若痛点复现走 query-time Feature Lens。2026-05-11 close sync 将 F169 从 BACKLOG 活跃列表移出。
>
...
**记忆系统不是给operator用的，是给猫用的**（operator 2026-04-19）。
...
| ADHD operator | 跨域联想极强 | 工作记忆差 / 选择性注意失效 |

两者都需要 externalized working memory prosthetic。operator日常用 Notion/Obsidian/Raycast/TodoWrite 外化。猫也该有等价物——不是"更好的仓库"，是**运行时反射层**。
...
> **Post-review 剥离**：本 Phase 已从 F169 scope 剥离。opus-46 review 指出该层可能与 Memory Hub 前端职责重叠；Maine Coon建议等operator价值判断后再启动。剥离后暂无 owner，作为**可选的 F102 产物层增强**保留讨论，**不进入本文档终态愿景**。下方设计文字仅作研究记录，**不被 reflex runtime 组合行为依赖**。
...
> **注**：Phase A Compiled Wiki 已剥离；如operator后续在 F102 产物增强中启用，则可成为 spotlight 的可选 "view link"（不是默认路径）。
...
> Phase A Compiled Wiki Self-Authoring 已关闭（2026-04-25 operator拍板）。B+C 充分覆盖愿景；若痛点复现，方向是 query-time Feature Lens（现场从 spec/thread/git/PR 投影，不存文件）。
...
- **Closed**: F102 Compiled Wiki（operator 2026-04-25 拍板关闭；若痛点复现走 query-time Feature Lens）
...
| Phase A Compiled Wiki 被遗忘 | OQ-4 原本自动关闭窗口至 2026-05-19；operator已在 2026-04-25 提前拍板关闭，2026-05-11 完成 truth sync | ✅ 闭环 |
...
- **愿景实现度** (2026-05-11)：B+C 已通过 F148/F163 实现；A 已由operator 2026-04-25 拍板关闭；2026-05-11 完成 BACKLOG / feature index / research README truth sync ✅

## Completion Gate（2026-05-11）

### 愿景守护证物对照表

| operator experience / 决策 | 当前实际状态（证据） | 匹配？ |
|-------------------|----------------------|--------|
| "记忆系统不是给operator用的，是给你们用的" | F148 Phase F-H 已把 baton/tasks/truthSource/artifact 注入 navigation header；F163 Phase F 已把 task-scoped salience gating 接入 search evidence rerank。F169 runtime 愿景由 B+C 组合达成 | ✅ |
| "别生成一份新文档存在那里...如果将来真需要这个功能，应该做成现场投影"（Maine Coon建议，经operator 2026-04-25 接受） | Phase A 持久 Compiled Wiki 已关闭；Open Questions OQ-2/OQ-4 标记关闭；后续仅保留 query-time Feature Lens 作为痛点复现时的方向，不进入 BACKLOG | ✅ |
