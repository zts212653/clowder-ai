...
>
> **Reverted**: operator 2026-04-26 01:05 否决 ——
> 1. **完全理解错原 bug**：原始问题 [thread-id] 是"前端连他们的头像、CLI thinking 什么都看不到"——整个 ChatMessage 组件没渲染（DOM 缺失）。F176 把它误诊为"内容被折叠"，做了 messageRole 分流。**修了一个不存在的 bug，没修真 bug**。
> 2. **视觉效果丑**：F176 的"主气泡 + 独立 CliOutputBlock 卡片"渲染分流让气泡裂开，operator experience："你们现在这个气泡渲染行为 太丑了"。
> 3. PR #1401 + #1402 全部 revert，messageRole 字段 + 所有相关代码移除。
>
> **真 bug 仍未解决**：[thread-id] 里 opus/codex 互 @ 后 ChatMessage 整体不渲染（头像 + CLI Output 都没出来）。需要单独立项重新诊断（候选 F177）。
>
...
>
> **Triggered by**: `[thread-id]` 实测（2026-04-25 13:14），operator报告"前端看到互相调用但看不到说话气泡"。@codex Maine Coon（GPT-5.5）+ @opus47 Ragdoll（Opus-47）双独立诊断收敛到同一根因（5/5 一致）—— **但收敛到的是同一个错误根因**。
>
...
## ⚠️ Postmortem 反思（operator 2026-04-26 01:10 让写）🔴
...
**这一段的第 4 行就错了。** operator原图里 opus/codex 互 @ 的位置**根本没有 CLI Output 卡片**，只有 BriefingCard + DirectionPill 横标签。**整个 ChatMessage 组件没渲染**（DOM 缺失）。
...
### operator的原始意图（2026-04-26 01:02 + 01:05 三个感叹号才纠正过来）

> "我滴吗 这个f176 你们完全理解错了啊，当时是为了修这个bug的，就是Ragdoll和Maine Coon互相 at 然后互相说话了，但是我前端连他们的头像 cli thinking 什么都看不到！！"

operator说"看不到说话气泡" = **整条 ChatMessage 不渲染（连头像、连 CLI thinking 都没出来）**，不是"内容被 CliOutputBlock 折叠"。我把"看不到"误读为"被折叠看不到"，是把同一个动词的两种语义搞混了：
- 真意：DOM 缺失（看不到 = 不存在）
- 误读：UI 折叠（看不到 = 默认隐藏）
...
3. **quality-gate Step 0 愿景对照表**也按错误前提走：我对照"operator experience"，但**对照的不是图，是我自己写的 spec**——spec 里把"看不到"已经误读成"被折叠"。
4. **AC-E1 alpha 验收**最后由 sonnet 接力做，但他验证的是"messageRole 是否端到端透传到主气泡"——**不是验证"[thread-id] 现象消失"**。AC-E1 写的"现象消失"被偷换成了"messageRole 工作"。
...
1. **Spec § Why 必须基于图/原话 verbatim quote**，不能加自己解读。如果原话有歧义 → 反问operator澄清，不自己拍板。
...
> ```
> operator在 [thread-id] 看到（图: thread.png）：
...
### 端到端
- [ ] AC-E1: `[thread-id]` 现象消失——codex/opus native CLI 主回复显示主气泡
...
- 端到端：alpha 拉新 thread 复现 [thread-id] 场景，验证主气泡显示 + CLI 折叠保留
...
- **2026-04-25 13:14** operator报告 [thread-id] 看不到说话气泡
- **2026-04-25 13:18** 双猫并行诊断收敛同一根因（5/5 一致）
- **2026-04-25 13:22** 提出 messageRole 完整方案 + F173 共存策略
- **2026-04-25 13:36** operator ack 立项 + 给号 F176
