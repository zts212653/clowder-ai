...
# F110: 训练营愿景引导增强 — operator 需求挖掘 + SOP 显式加载

> **Status**: spec | **Owner**: Ragdoll | **Priority**: P1
> **Evolved from**: F087（operator Bootcamp — 训练营基础设施已完成，但 Phase 5+ 引导能力不足）
...
1. **没有愿景采访流程** — Phase 5 只有一句"用户说愿景，猫猫帮结构化成 spec"，没有引导性问题模板。新手operator需求表达不清楚时，猫猫不知道怎么引导
...
> "你这新手训练营 会主动提醒Ragdoll加载sop 引导新手指挥官 按照对齐愿景来吗？ 会采访新手operator他的愿景要如何提吗？很多人需求都特么表达不清楚 我们的 skills 里有写让你们去揣摩用户隐藏需求然后 引导去问他们要不要吗？"
...
| 4→5 过渡 | 回放结果+桥接仪式 | 庆祝就任operator+分工说明消除畏难 | **回放+庆祝仪式+分工说明（不做入口分叉）** |
...
2. **庆祝就任 operator 仪式**（配合 Rich Block 卡片/语音）：
   ```
   🎓 恭喜上任 operator！从现在起——
...
每个 Phase 转换时，猫猫要用 **「🎯 operator 决策时刻」** 标注，并用一句白话解释为什么需要人类判断。
...
|----|---------------------------|---------|----------|------|
| R1 | "会采访新手operator他的愿景要如何提吗？" | AC-A1, AC-A3 | skill 文本 review | [ ] |
...
- **Evolved from**: F087（operator Bootcamp — 基础设施）
- **Related**: F106（多训练营支持）
...
| KD-8 | Maine Coon建议的"利害关系人冲突"和"风险容忍度"不纳入训练营 | 训练营是单人 operator + 轻量项目，不适用 | 2026-03-12 | Ragdoll判断 |
| KD-9 | Phase 3.5 可选功能：主动帮装，不跳过。根据用户硬件配置推荐 TTS/ASR 方案 | 实际用户反馈：env check 通过后猫猫把可选功能当"不重要"直接跳过，用户想装但没被引导。我们自己用 Qwen3 系列 TTS，但轻量场景推荐 Kokoro-82M；需要根据用户硬件（Apple Silicon/NVIDIA GPU/CPU-only）做差异化推荐 | 2026-03-13 | operator指出 |
| KD-10 | 当前训练营定位为"新手训练营"，Phase 3.5 帮装可选功能属于新手范围 | 进阶训练营（如自定义 LLM 后训模型、高级 pipeline 配置等）留 OQ 口子，不在 F110 范围内 | 2026-03-13 | operator指出 |
