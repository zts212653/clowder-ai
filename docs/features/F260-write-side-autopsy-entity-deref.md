...
**F316 Phase B AC-B2 bounded contribution（2026-09-04，F260 owner 裁定）**：F316 Phase A census（PR #4320）发现 Entity 缺真实调用三态矩阵（resolved / not_available / ambiguous）+ 旧 revision 失效证明；Phase A receipt 由 codex-sol 代答（accepted-gap / pending-owner-custody），本条为 owner 正式处分。**裁定：bounded contribution custody → F316 Phase B carrier**（`[thread-id]`，执行 codex-sol、非作者 review codex-terra，一个 Phase B 主 PR，不开 F260 支线——同 2026-07-27 operator"一个问题一条施工线"原则；投递 `0001788533190639-001219-07fa171e`，coordination terminal）。**F260 authority 保留为验收契约六条**（Phase B 交付按此审、owner 事后核对）：① 三态各 ≥1 真实或可 replay 调用，绑定 exact `entity_registry` revision（非 mock 表）② `doc_aliases` 命中永不进 Person lane——KD-5 分表的设计意图（文档索引 ≠ 实体注册表），需阴性测试 ③ canonicalName 修正或 retire 后，旧 revision 的 resolved 结果必须失效（先例：ep-5 家属喵 v2 覆盖 v1）④ Entity 只持 workspace identity root，零读写 F276 owner-private（KD-7 / M21 两种日记分道）⑤ 不动 detector 阈值、不引 NER 或人名 regex（与 #3262 同口径）⑥ PR 内先 RED 再 GREEN 语义 commit，本 spec AC-B2 验收记录行随该 PR 追加。

**F316 Phase B AC-B2 execution evidence**：初始 RED `04af580d03` 在 real SQLite registry 上暴露 typed outcome / revision 缺失；初始 GREEN `22925c51c1` 增加 `resolveExactAliasOutcome`、visible-registry revision 与 per-identity projection revision，并把 identity revision 带入 `DetectedEntity → NudgePayload` typed metadata。非作者 review `[thread-id]#0001788550378768-001488-78930b42` 继续发现该 revision 在 `subject_seen` seed 边界丢失，排队旧结果可在 correction/retirement 后打开 Person cue；review RED `0ddfc80cd7` 复现，GREEN `5ef6ca54fc` 将 exact revision 带入严格 carrier，并在 owner-private Person read 前后以 active + viewer-visible Entity projection 双重 fail closed。最终 matrix 5/5、shared carrier 16/16、Entity/Person boundary 29/29、F260/Entity focused 129/129；私密 create/correction/retirement 不改变 outsider 的 empty/not_available revision，owner revision 正常改变。未改 detector 阈值、未引 NER/人名 regex、未写 F276 owner-private store。Terra 对 exact HEAD `ee8c9915656e214f2ba57cb5f6cd327235288002` 给出 typed approval（`[thread-id]#0001788555692984-001546-dfb3b77c`）；PR #4327 已以 squash `99c9b40a55cac23174dd9ebc10753b5f62576cc4` 合入 main。
