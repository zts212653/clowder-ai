...
Cycle 1 的唯一变量已经收窄到 F100 owner 明确认领的资产边界：`assetKind=skill`、`assetId=cat-cafe-skills/request-review/SKILL.md`、范围仅为 accepted-source anchor 模板 / 字段 / re-ack。Owner source：`[thread-id]#0001788669097832-001229-10130bb6`。当前仓库可复核该文件最近内容 commit 为 `1aff39f464261971eed9bc6722a53a1eb81226ac`、当前 blob 为 `3c5902e12467602bd9f9ae81f098ade05c92e4ea`；这只是可重开源码 revision，**不是** F311 所需的 owner-signed `ExactAssetVersionRef`、current proof、adoption proof 或 actual-use receipt。

责任与停止条件如下，禁止用一个 carrier 冒充另一个 owner：

| 范围 | Canonical carrier / holder | 停止条件 |
|---|---|---|
| F314 integration / acceptance | Task `0001788363187122-000197-b066434c` / `@codex-sol`（doing） | AC-C5：首次成熟 `keep/sunset`；若为 `tune`，还需 owner mutation receipt + fresh outcome |
| F311 Workspace consumption / Alpha acceptance | Task `0001788629040413-000566-66871cd1` / `@codex-astra`（doing） | owner-backed `versionReview` 已在新 Alpha 由原 F307 history 读通；AC-UX4 仍须下一项真实同类任务产生 actual-use/source/revert 证据，接口验收不替它关闭 |
| F314 observation join | Task `0001788502777777-000280-36ac2f17` / `@codex-sol`（doing） | canonical trajectory、异质 owner surfaces 与 F267 proof 成为 Program 可消费 refs；它不生产资产版本 |
| F100 exact-version publication + owner reader | Task `0001788670168158-001250-2aae7658` / `@opus`（done）；PR #4435 / merge `fce8043d0e16b90fe6f94b2e2757b785832378ec` | Reader 已在 main 注册进 `ProgramAdapterRegistry`，33/33 tests green；live runtime 仍 dormant。该交付只关闭 reader implementation，不替代 owner-loop integration、adoption 或 actual-use proof |
| F314 owner-loop integration | Child Task `0001789208136313-000123-b0eb871a` / `@codex-sol`（done）；PR #4517 / merge `2baede2cf314e4f99fc6346d4ba773c67ddbff26`，反向锚定 parent Task `0001788363187122-000197-b066434c` | F100 语义版本/history/parent/diff、exact lineage owner actions、三类 evidence、actual-use 与恢复接口已落地；新 Alpha 对同一 `ba0f…` Program 的 owner/F307 读回与重启恢复通过。自然 adoption/use/fresh outcome 不存在，不能由接口验收补造 |

F100 owner 的 reader implementation 已由 PR #4435 合入 main，owner-loop interface 随 PR #4517 与新 Alpha 同源消费验收闭合；`reader_pending` 与接口 child Task 均已终结。**接口闭合仍不等于 F314 business loop 已闭合。** Parent Task `0001788363187122-000197-b066434c` 继续由 F314 持有，observation Task `0001788502777777-000280-36ac2f17` 仍独立推进；它们等待自然证据、必要批准、实际采用与 fresh outcome。F314 不把 Git commit、manifest、Alpha deterministic replay 或 owner read 冒充 adoption/use，已完成的 wiring/reader/interface Task 也不得被复活承接这些业务条件。
