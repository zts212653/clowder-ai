...
tips_exempt: "Renewed 2026-09-04 for inline recovery usability: the source-bound card itself teaches the owner which authorized conversation is newest, lets them inspect each exact conversation, and names the bind-and-send side effect; a separate capability tip would duplicate the in-place recovery surface."
...
- [x] AC-F4: `@gpt-pro` 因未绑定而零发送时，在原 source message 旁生成唯一 inline recovery card。单授权会话预选；多授权按最近授权排序，有安全标题时显示标题，否则显示授权时间 + 短 ID，并逐项提供用户触发的 canonical“打开查看”；零授权提供用户触发的 ChatGPT 授权入口。主动作“绑定此会话并发送”必须先写 exact owner-only thread route，再复用 original `sourceMessageId` + current retryable `attemptId` 继续同一 queue target；prop 缺 fence 时只可 owner-only exact-source 补读，POST 仍原子复验；未获点击授权时不创建浏览器 delivery，重复点击/重载/重放不得克隆 source 或产生第二个 Host send（2026-09-04 deterministic component/API PASS；live Alpha pending）
