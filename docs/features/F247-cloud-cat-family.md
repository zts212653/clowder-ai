...
tips_exempt: "Renewed 2026-09-02 for the v31 assistant-remount recovery and durable observation diagnostics: this repairs response loss inside the existing owner-only Developer Preview and adds no public Chrome Web Store listing or teachable UI entry; add a tip only after signed publication creates a public onboarding surface."
...
Architecture cell: plugin + callback-auth + transport + dispatch + bubble-pipeline
Map delta: v24 keeps MCP as the primary source-return writer and adds one browser fallback inside the existing plugin + callback-auth + dispatch + transport cells: after the exact newly observed user turn, the page adapter may read only the first causally following assistant turn, the Native Host holds it in the mode-0600 delivery ledger, and the API competes through the same durable exact-source idempotency key as MCP. Host receipt observation remains bounded to one conversation turn, accepts only one globally unique Host-provided ID, and grants no history-wide search authority. The dedicated gpt-pro process binds its one authorized principal server-side; shared multi-key runtimes still require an explicit selector, and no principal, thread scope, replacement, review, coordination, action, focus mutation, or external origin authority is added (last updated 2026-09-01).
...
## Current State / 基线（截至 2026-09-01）
