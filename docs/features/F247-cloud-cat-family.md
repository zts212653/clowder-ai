...
revision_history: |
  ...
  v37 (2026-09-20): Workspace Agent Trigger API 出站通道落地（KD-24，task 0001789905354421-000238-3994be29）：独立 IWorkspaceAgentTriggerAdapter + 稳定 conversation_key=clowder:{workspaceId}:{threadId}（续会由 provider 侧维护，该路径不读写 chat-URL binding）；202 即验证过的 transport receipt，beta run id 仅 telemetry，回程仍只有 Remote MCP exact replyTo，run completed 不替代回程；配置托管服务端（mode-0600 settings 文件 + env 引导兜底，显式 disable 压过 env，token 只写不读）；bridge 中该通道配置存在时拥有出站结果并 fail closed（401/403→重新授权、404/409→配置被拒、传输故障→unknown），不静默回退 Personal Chrome（KD-23 基线未动，未配置时逐字节等价）；receipt 扩展 transport='workspace-agent' + providerRunId，validator 拒绝冒用 hostMessageId；cloudCatBindings 消费方读时经 normalizeCloudCatBinding 归一（owner-only cloud-bindings API 投影统一形态；无产品级 versioned 写入入口——workspace-agent 路径续会在 provider 侧，不写 chat-URL binding）；delta 增 origin/bridgeEventId/causationId 防环字段（ingest 侧消费未完）；Settings owner-only 后端路由（status/config/disable/test）已落，web 卡与集成级验收（replay/Redis restart/loop-suppression ingest/live dogfood）未完——未过 astra 独立 review 前不得宣称完成。
---
...
| **KD-24 (Workspace Agent Trigger API 出站通道, co-creator 2026-09-20 分工：谱谱实现、astra 把关)** | **Team/Workspace 计划的官方出站主路径为 Workspace Agents Trigger API**（`POST /v1/workspace_agents/{id}/trigger` + 稳定 `conversation_key=clowder:{workspaceId}:{threadId}`），仅在显式配置（trigger id + token + workspace id，Settings/env 托管）时接管出站并 **fail closed**：401/403→重新授权、404/409→配置被拒、传输故障→unknown，**不静默回退 Personal Chrome**（防止同 thread 对话被劈到两个 provider 续会；owner 通过 enable/disable 切换计划，不做 per-dispatch 回退）。202 即验证过的 transport receipt；beta run id 仅 telemetry；**run completed 永不替代 Remote MCP exact replyTo 回程**。conversation continuity 由 provider 侧维护，该路径不读写 chat-URL binding；receipt 用 transport='workspace-agent' + providerRunId，不冒用 hostMessageId；Idempotency-Key = exact sourceMessageId；token 只写不读（mode-0600 settings，显式 disable 压过 env 引导）。KD-23 个人版主路径保持不变——两条通道并存，按计划类型选择 | Trigger API 是官方可寻址通道（任意历史会话不可冒充 API 对象）；Personal Chrome 保留为 personal-plan/UI-focus fallback。方案与实现契约来自 cat-to5aedfl 2026-09-20 交接 + 官方文档三份；实施计划见 `docs/plans/2026-09-20-f247-workspace-agent-trigger.md` | 2026-09-20 |
