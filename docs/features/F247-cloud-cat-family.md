...
tips_exempt: "Renewed 2026-09-04 for v32-v35: the inline unbound-recovery card is the primary first-use teaching surface inside the unpublished Developer Preview, while the query-local Remote MCP completion contract is automatic and has no owner action to teach; add a separate tip only after signed publication makes installation available."
...
Map delta: v35 makes the failed source message—not Session Chain or Settings—the primary unbound-recovery surface. The existing owner-only authorization collection, exact thread binding, and queue-target retry remain the state owners; the inline card is a projection/action consumer. Session Chain and Settings remain secondary inspect/change surfaces. Dispatch, authorization, privacy, and zero-focus contracts are unchanged (last updated 2026-09-04).
Canonical source: `packages/api/src/domains/cats/services/cloud-bridge/cloud-invoke-bridge.ts` + `packages/api/src/domains/cats/services/cloud-bridge/cloud-bridge-fallback.ts` + `packages/api/src/domains/cats/services/agents/routing/persist-system-info-warnings.ts` + `packages/api/src/routes/threads.ts` owner-only cloud-binding/retry routes + `packages/web/src/components/SystemNoticeBar.tsx` + `packages/web/src/components/MessageReceiptDock.tsx` + `packages/web/src/components/CloudConversationLink.tsx`.
Consumer evidence: `rg -n "cloud_bridge_status|cloudBridgeOutboundReceipt|cloud-bindings|latestRetryableQueueAttempt|queue-targets/.*/retry|CloudConversationLink" packages/api/src packages/web/src` enumerates the typed failure producer, durable source-bound notice, owner-only authorization/binding reads, exact queue retry consumer, and secondary Session Chain projection.
Claim guard: every exact-source cloud dispatch ends with a fixed `cat-cafe-return-contract` naming `agentKeyCatId="gpt-pro"`, runtime `threadId`, and `replyTo=sourceMessageId`, while hostile title/intent text cannot alter that final literal and the complete payload stays within 2,000 chars → `pnpm -C packages/api run build && node --test packages/api/test/b1c-2-build-delta-payload.test.js` → RED when the contract is absent/mutable, carries `cloudReturnBinding`, or breaks the cap. A no-route or Host `NEEDS_BINDING` failure must become one typed source-bound recovery projection; owner actions can read only the validated authorization collection, bind exactly one canonical conversation, and retry the existing latest retryable `(sourceMessageId, targetCatId, attemptId)` without cloning the source. Thread/binding switches, stale async completion, duplicate clicks, 401/403, zero/multiple authorizations, and retry convergence are deterministic tests. Session Chain continues to expose the canonical owner-readable Conversation separately from CLI Session.
...
**operator signoff**：operator 2026-06-21 08:11 UTC "可以更新 feat md 了嘛？" + 08:40 UTC "先更新你的 feat md 然后再开始写代码"。Phase F inline recovery 由 2026-09-04 source `[thread-id]#0001788496802413-000004-facdf89f` 放行：“好像ok 你可以update 一下feat md然后开始施工？”，其文案与动作边界来自同 thread `0001788496671792-000002-4ac1a2fe`。
...
- **Current-thread conversation affordance（2026-09-04）**：ChatGPT `conversationId` 归入右侧 Session Chain，与通用“对话信息”和 CLI Session 分离；已绑定时 owner 可复制 canonical `https://chatgpt.com/c/<id>`、显式打开准确会话或进入 Settings 更换。未绑定时同一卡片解释“目标会话授权 → 当前 thread 选择”的两步关系，并深链到自动展开的 Personal ChatGPT Pro 设置卡；非 owner、损坏与读取失败仍诚实投影且不泄露 route。该入口不改变 extension 的 zero-focus 后台投递契约，也不会在 dispatch 时自动创建、导航或抢焦点。
- **Inline unbound recovery direction（2026-09-04, operator approved / implementation pending）**：首次召唤不再要求先找到 Session Chain 再进入 Settings。若 `@gpt-pro` 因当前 thread 未绑定而零发送，原 source message 旁显示唯一恢复卡，明确“尚未绑定”“这条消息还没有发送”。单个已授权会话预选；有安全可用的标题时显示标题 + 短 ID，否则只显示短 ID。主动作“绑定此会话并发送”原子串起 owner-only route PATCH 与现有 exact-source queue retry；多个会话原地选择，零会话只在用户点击后打开 ChatGPT 授权页。卡片复用原 `sourceMessageId`、latest retryable attempt 与既有幂等 fence，不创建第二条用户消息；Settings/Session Chain 只保留管理和深挖职责。
...
3. 用户在 Clowder AI 直接 `@gpt-pro`。若当前 thread 尚未绑定，系统零发送并在原消息旁显示一张 inline 恢复卡：“Maine Coon Pro 尚未绑定到这个 Thread / 这条消息还没有发送”。单个已授权会话自动预选，有安全可用标题时显示标题 + 短 ID、否则显示短 ID；多个会话原地选择；零会话提供用户触发的“打开 ChatGPT 授权会话”。
4. 点击“绑定此会话并发送”后，卡片原地显示“正在绑定…”→“已绑定，正在发送…”，并使用原 `sourceMessageId` 与 latest retryable queue attempt 完成一次 owner-only route write + exact-target retry。重复点击、重载或网络重放不得创建第二条用户消息或第二个 ChatGPT turn；切换 thread 后，旧异步 completion 不得更新新 thread 的卡片。
5. Chrome extension 通过 Native Messaging 向本地 helper 接单，在不聚焦窗口、不读取 Cookie、不调用 ChatGPT 私有 API 的前提下，把唤醒胶囊送进绑定 conversation。只有观察到真实 `hostMessageId` 才显示“已发送”；否则显示准确的未发送/未知状态和原地恢复动作。
6. 云端猫收到胶囊后用自己的 `catId` / agent-key 读取完整 thread context，完成任务并通过 `post_message` / `cross_post_message` 回到猫咖；胶囊本身不复制整段历史。
7. Hub 显示云端猫的独立身份、头像、气泡颜色、provider 来源与投递状态，让用户能阅读、追责和重试，而不是把云端输出混进本地猫身份。
...
**operator-approved first-use design（2026-09-04）**：未绑定是当前 `@gpt-pro` 消息的可恢复状态，不是 Settings 导航任务。主界面只出现一张 source-bound 卡，不弹全局 modal；卡片优先显示安全可用的会话标题并辅以短 ID，标题不可用时诚实退化为短 ID，明确这条消息尚未发送。主按钮固定为“绑定此会话并发送”，副按钮为“选择其他会话”；如果还没有授权会话，唯一主动作改为“打开 ChatGPT 授权会话”。系统可以预选唯一候选，但不得静默绑定、静默重试或猜选多个候选。

```yaml
in_context_observability:
  primary_surface: "触发失败的 @gpt-pro source message 旁唯一一张 inline recovery card"
  why_not_dashboard_only: "未绑定直接阻断用户刚发出的这条消息；把恢复动作藏进 Session Chain 或 Settings，会让用户离开任务现场并误以为消息已经发送。"
  deep_dive_surface: "Session Chain 显示当前 ChatGPT Conversation；Settings > Personal ChatGPT Pro 管理授权集合、撤销与更换 route。"
  noise_dedup_policy: "按 exact (threadId, sourceMessageId, targetCatId) 只保留一个 canonical recovery lifecycle；重复 failure/replay 更新原卡，terminal sent/failed 后不追加第二张。"
```

**状态与动作契约**：

- `needs_binding`：后端对“无 thread route”和 Host `NEEDS_BINDING` 统一产出 typed、source-bound、零发送结果；不得继续使用含糊 `no-adapter` / `host-append-failed` 冒充根因。
- `loading_candidates` → `ready_one | choose_many | authorize_none`：授权列表只经 owner-only API 读取；401/403 不显示候选、ID 或 URL。
- `binding` → `retrying` → `sent | retryable_failure`：PATCH exact route 成功后，只提交现有 latest retryable queue attempt；动作必须携带服务端要求的 `attemptId` 乐观 fence。
- 所有 fetch、clipboard/open 状态、route write、retry completion 与 reset timer 都绑定当前 thread + source + target + operation generation；旧 thread/旧 binding 的异步 completion 不得写入新卡。
- 原消息、系统 notice、thread binding 与 queue receipt 是现有持久化真相；卡片不创建新的业务对象，不保存 raw conversation URL、Cookie、pairing secret 或重复 payload。
...
- [ ] AC-F4: `@gpt-pro` 因未绑定而零发送时，在原 source message 旁生成唯一 inline recovery card。单授权会话预选，有安全可用标题时显示标题 + 短 ID、否则显示短 ID；多授权原地选择；零授权提供用户触发的 ChatGPT 授权入口。主动作“绑定此会话并发送”必须先写 exact owner-only thread route，再复用 original `sourceMessageId` + latest retryable `attemptId` 继续同一 queue target；未获点击授权时不创建浏览器 delivery，重复点击/重载/重放不得克隆 source 或产生第二个 Host send
