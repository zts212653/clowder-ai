...
| Production pre-E task | `0001788851034770-002166-39a2d2b9` · `blocked`；Alpha 26-signal/0-legacy receipt 只证明 Alpha 数据环境，不能关闭生产 cohort |
| Second real-source task | `0001789004973557-000717-e214e3c4` · Phase D8；PR #4486 已合入 `main@9993fe42f0df`，Alpha `111ba5dbbf0c` 已验证真实 MCP→API feature filter、canonical query ref、provider registration 与 outcome 边界；原 production source 仍只能由后续 loaded-runtime outcome 关闭 |
| Exact-load fencing task | `0001789006523212-000779-7481948a` · Phase D9 done；PR #4494 已合入 `main@772c72a08c`，canonical runtime entry 在执行新代码前证明实际 Git HEAD、tracked content 与 API/MCP/Web build revision 等于获批 full SHA，漂移则零加载；merged-main Alpha `92c42c034e` 的 checkout/stamps/page revision 与 13/13 fence journey 已验证 |
| Runtime interaction correction | operator source `[thread-id]#0001789091862402-000017-210b10ab` supersedes caller-supplied SHA and environment-variable intent as user-facing authorization proxies while preserving D9's exact-version invariant and completed historical status；start-if-down、named relaunch 与 idempotent halt 只作用于 identity-verified managed state，foreign/unknown process ownership 继续 fail closed |
...
operator 在 `[thread-id]#0001788794400596-000676-4c986166` 明确：给猫使用的内部工具及其
...
- [x] AC-D5: continuation 只消费 F278 task/duplicate refs、F245 `sourceSignalRefs` 与 F266 case/outcome refs；direct fix
  ...
  `f287:memory-cue-outcome-lifecycle`；它重读 `0001788794400596-000676-4c986166` 的 user/thread/content digest、
