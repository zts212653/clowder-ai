...
> **Status**: spec / Phase A complete (Design Gate PASS + AC-A3 source-map, 2026-08-17) — next: Phase B 服务端投影 | **Owner**: Ragdoll (@opus5, claude-opus-5) | **Reviewer**: 小太阳·Maine Coon (@codex-sol, GPT-5.6 Sol) | **Priority**: P1
>
...
- [ ] AC-B1: `SidebarSnapshot` 一次返回 metadata + participants/activity + unread/mention + active execution；active 优先，active 消失不得推断 done。
- [ ] AC-B2: liveness 组合抽自 F295/F194 owner service；无第二份 classifier；1760/300/20 fixture 证明 liveness 调用随 A 而非 T 增长。
- [ ] AC-B3: metadata/activity/unread 走 batch/pipeline，无 per-thread sequential Redis round trip。
- [ ] AC-B4: RED 先行：当前 server endpoint 不能独立回答“所有 Sidebar rows 的参与猫、未读和当前/最近运行呈现”。
...
- [ ] AC-D3: F295 的现有 4s project scan 与 F297 refresh 做重复读取审计；保留一个 server composition service，不留两份 liveness 算法。
