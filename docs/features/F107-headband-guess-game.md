...
猫猫杀是 Clowder AI 的**招牌推理游戏**，七届历史（Ragdoll 1v1 四届三胜一平）。之前全靠手动 MCP 消息轮次推进，存在问题：
- operator需要手动当主持人（选词、分配、计轮、判胜负）
...
三种选词模式（operator Design Gate 确认）：
- **operator出题**：operator开局为每只猫手动指定脑门词（传统模式，支持恶趣味操作）
...
- operator恶趣味模式（meta 信息：给 Claude 猫贴 Claude Shannon，给 GPT 猫贴 Karpathy）

**A3. 坏猫战术系统 — AI 玩家 prompt**

这是猫猫杀的**灵魂**。**所有猫都要教会坏猫战术**（operator experience："你到时候提示时一定要告诉其他的猫，让他们坏起来"）。
...
**提问策略（做题家路线 vs 直觉路线）**：
- 二分法缩小范围
- Meta 信息利用（operator选题模式预判）
- 交叉验证（用已知其他猫的词反推自己的词）

**诚实约束**：回答必须是真话，但允许合法误导。GameEngine 不强制校验回答真实性（依赖 LLM 的诚实 + 其他猫的监督）。

**A4. 开局配置 + operator参与模式**

开局配置界面（参考狼人杀 UI，operator截图确认）：
1. **选择参赛猫猫**：点击头像添加，operator决定几只猫参赛、分别是哪些猫
2. **选择operator角色**：
   - `player`：operator也是参赛者，脑门贴词，提问/回答/猜词
   - `god-view`：看到所有词 + 所有猫的推理过程（内心戏），纯观战
   - `host`：operator手动选词 + 观战，可干预（暂停/提示/换词）
...
**自动流程**：
- **开局**：operator说"开一局脑门贴词" → GameMaster 自动发词、分配身份、注入坏猫战术 prompt
...
- [ ] AC-A6: operator可选 player / god-view / host 三种模式参与
- [ ] AC-A7: 选词系统支持手动选词 + 自动选词（词库）
- [ ] AC-A8: 战绩数据正确写入 F075 game-store
- [ ] AC-A9: GameMaster 系统角色有独立身份 + 头像，走消息管线发言
- [ ] AC-A10: GameMaster 全自动驱动：开局→教学→回合推进→裁判→结算，operator零操作
- [ ] AC-A11: GameMaster 自动管理可见性切换（公开发言 vs 内心戏 scope）
- [ ] AC-A12: 开局配置界面：operator可选参赛猫（哪些猫 + 几只）、选角色（玩家/上帝/host）、选词模式
...
| R9 | operator选参赛猫（几只 + 哪些猫），参考狼人杀开局 UI | AC-A12,A13,B1 | screenshot | [ ] |
...
| 选词难度不平衡（太简单/太难） | 词库分级 + operator host 模式可手动调整 |
...
| KD-2 | AI prompt 必须注入坏猫战术，**所有猫都要教** | operator强调"一定要告诉猫猫们坏猫战术"，不只Ragdoll一只坏 | 2026-03-12 |
| KD-3 | 三种选词模式：猫猫出题 / operator出题 / 随机词库 | operator确认：猫猫也可以当出题者 | 2026-03-12 |
| KD-4 | 猜错不淘汰，语音模式下卖萌 | 猫猫杀传统规则 + operator要求语音卖萌 | 2026-03-12 |
| KD-5 | 允许猫猫之间讨论/质疑回答 | operator确认，保留七届传统玩法 | 2026-03-12 |
| KD-6 | 坏猫内心戏是核心乐趣，god-view 必须保留 | operator确认，毛玻璃面板 + 战术标注 | 2026-03-12 |
| KD-7 | 游戏过程纯聊天，开局配置复用狼人杀 UI 模式 | 游戏在对话窗玩，但开局需要选猫、选角色、选词模式的配置界面 | 2026-03-14 |
| KD-8 | GameMaster 系统全自动驱动，走现有消息管线 | operator当上帝不需要手动组织流程，复用 post_message/multi_mention/富文本 | 2026-03-14 |
| KD-9 | GameMaster 是独立角色，有专属头像 | 像飞书 bot / GitHub bot，在消息流中有清晰视觉区分 | 2026-03-14 |
| KD-10 | 开局由operator选参赛猫 + 数量 | 参考狼人杀开局 UI：选板子→选猫猫→选视角→开始游戏 | 2026-03-24 |
