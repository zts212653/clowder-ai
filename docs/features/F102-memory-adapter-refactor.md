...
猫猫如果不知道 Knowledge Feed 存在，就不会主动提议沉淀知识、不会帮team lead打开 Feed。需要：

1. **CLAUDE.md/AGENTS.md 更新** — 在记忆系统段落加"知识涌现 Feed 在 Workspace 面板里"
2. **workspace-navigator 扩展** — 支持"打开知识 Feed"指令
3. **猫猫主动提议的 prompt guidance** — 在系统提示词里告诉猫"对话中发现有价值的 decision/lesson 时，主动提醒team lead"

> **下一步**：team lead确认产品原则 → Siamese(Siamese)出精细视觉设计 → 实现
