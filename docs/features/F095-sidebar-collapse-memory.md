...
operator反馈：Thread Sidebar 在当前 5-10 个项目时已经很难用（cat-cafe 174 条 thread 却排在第四位），新建对话窗口太小、不能命名、不能关联 feat、不能置顶。
...
**沉痛教训**：operator误删 `[thread-id]`（73 条审计记录，cross-thread-sync 教训 thread），不可恢复。
...
参考 Codex App 的项目管理体验（operator截图 2026-03-27）。
...
- [ ] AC-D6: deletedAt 超过 30 天的 thread 自动物理清理 — **延后**（需 cron 基建，operator确认后续再做）
...
| R3 | 搜到了但分组折叠看不到（Maine Coon发现）；搜索态手动折叠不生效（operator 2026-06-01 报告，PR #2022 修复） | AC-A5 | test | [x] |
...
| R10 | operator误删 thread 不可恢复（沉痛教训） | AC-D1~D7 | test + manual | [x] |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 从"折叠记忆"扩展为"导航体验升级" | operator要求面向 50 项目终态设计（家规 P1） | 2026-03-11 |
...
- Phase B: Design Gate（前端 UX → operator确认 wireframe）→ Ragdoll实现 → Maine Coon review
- Phase C: Design Gate（前端 UX → operator确认 wireframe）→ Ragdoll实现 → Maine Coon review
