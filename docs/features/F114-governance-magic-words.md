...
> "我感觉做愿景守护的喵，要么他没有认真的看我的愿景，要么你们两只猫的人类意图理解能力有问题。但是这个问题在于，按照你跟Maine Coon的智力水平，这句话不应该理解错。" — operator 2026-03-13

> "我们的家规或者喵约里面很多很重要的该怎么办？" — operator 2026-03-13

> "如果一天就能写完，你还要做那些先搞一个垃圾版本……可能又搞了一周都没干完" — operator 2026-03-13

### 问题根因

1. **愿景守护流于形式**：F101（mode v2 / 狼人杀）案例——operator愿景是"删旧 mode，把狼人杀加入 mode"。两次愿景守护都没拦住：守护猫只审计了 checkbox，没有从用户视角验证体验。

2. **声明式规则的固有弱点**：shared-rules §9 写着"AC 全打勾 ≠ 完成"，但没有任何机制能验证猫是否真的执行了这个检查。

3. **operator缺少紧急拉闸手段**：猫走偏时，operator只能用自然语言描述问题，没有能快速触发特定行为模式的触发词。

### 解决什么

两件事，不多不少：
1. 给operator 4 个 **magic words** 作为手动拉闸手段
2. 把愿景守护从"审计 checkbox" 升级为 **必须交证物才放行的 Gate**

### 不做什么（v1 讨论后明确排除）

- ~~喵约瘦身到 5 条~~ — operator指出：从系统提示词移走 = 注意率从 70% 降到 1%，是开倒车
...
| **「星星罐子」** | P0 不可逆风险 | 停止新增副作用（不发新命令、不写新文件、不 push），等operator指示 |

> **定位**：Magic Words 是注意力锚点（operator手动拉闸），不是最终安全机制。最终约束以 Gate 为准。
...
- [x] AC-2: operator发送「星星罐子」时，猫停止新增副作用并等待指示
- [x] AC-3: `feat-lifecycle` 愿景守护步骤包含 BLOCKED 条件（缺对照表 = 不放行）
- [x] AC-4: 愿景守护输出包含"operator experience vs 实际状态"对照表格式
...
| KD-6 | 现有 shared-rules 17 条结构不动 | operator + Ragdoll + Maine Coon三方共识：当前结构是最优折中 | 2026-03-13 |

## Review Gate

- 跨家族 review（Maine Coon review SystemPromptBuilder + feat-lifecycle 改动）
- operator亲自验收（用真实 magic word 测试猫的反应）
...
**蚊帐机制**：magic word 表只放「下次一定」一个（operator好喊），但 F177 Phase A 的 quality-gate follow-up 字样扫描（`check-followup-tails.mjs`）同时覆盖语义同族关键词列表。
