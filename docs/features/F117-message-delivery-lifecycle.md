...
3. **actual admission**：per-thread drain 按 comparator 顺序领取 source entry；一次领取冻结该 source 本轮**当前空闲且可 admission**的 exact
   target subset；若所有 pending target 都空闲，这个 subset 就是完整 target set；忙碌 sibling 仍留在原 row，不能阻塞健康 target。每个已领取 target 先并发建立自己的持久 processing response receiver，并给每条实际输入 source 写 exact
   dispatchRef；随后每个 target 在同一 source claim 内独立从 `targets[]` 退役并启动自己的 provider，尚未完成
   handoff 的 siblings 继续由原 claim 保护。最后一个 target 退役或失败 target 恢复 pending 后，触发下一次
   try-drain，不等待健康 target 的 provider terminal。不满足条件的尝试无副作用返回，enqueue/cutover/执行终局仍会再次触发。Steer 领取的只是
   同一机制下的 singleton target set，不另造 dispatch 路径。不同 source 不在 Queue admission 中合并；view 只 touch
   对应 source×target；
...
| E4 | 狸花猫 Steer 无「立即发送，引导回复」；旧设置把 append 能力与排队/立即意图混为一项 | UI 与契约错误 | 默认发送策略收敛为「排队等待 / 引导当前回复」，放在输入框左侧 `+` 菜单并可在 idle/active 任意时刻调整；设置只保留持久的「本 Thread / 全局默认」，不再提供与 Steer 重复的「仅这一次」或「恢复继承」。active 时右侧只保留 Stop，输入正文后增加一个普通 Send，发送只 admission 一次，再由 Queue 按策略处理。Queue Steer 固定为「立即发送，引导回复 / 立即发送，中断回复」，多选成员并逐成员选策略；静态 client capability 随成员信息投影，不支持 guide 时才禁用引导并在 disabled action 的 hover/focus tooltip 说明原因；是否已有 active reply 不参与弹窗能力判断，出队时无 active parent 则由 canonical admission 降级为新 invocation；targetless 绑定与新增 targets 在同一 source entry 原子完成 |
...
- [x] AC-D2: multi-target 使用同一 `targets[]`；普通 drain 一次领取本轮当前空闲的 exact target subset 并并发 fan-out（全部空闲时即完整 set），忙碌 sibling 与 Steer/未读未领取 target 原位保留；每个 target 在 receiver/ref durable 后独立删除，空数组删 entry
