...
| KD-14（修订 KD-10） | 主入口从「每条消息常驻动作条」改为 **thread 级常驻「选择消息」**；消息级入口在 hover/focus 设备上降级为快捷方式（静止态不画、不占行），在 `(hover: none) and (pointer: coarse)` 触屏上保持常驻可点并保留预留行 | KD-10 的真正价值是“默认可发现且不劫持原生操作”，不是“绑定到每条消息”。operator 两次实测把常驻动作条判为“狗皮膏药”；固定 `pt-8` 占位对每条消息收税，而 thread 级入口在触屏/键盘/桌面上同样默认可达。**但 thread 级入口只覆盖「选择」**：纯触屏既无 hover 也无键盘 focus，把整条工具条藏起来会让回复/删除/编辑/分支变成不可达（remote review 在 #3757 指出），所以隐藏只作用于有 hover 的指点设备。判据必须两半都要：只写 `(hover: none)` 会把「没有指点设备」的环境（headless 浏览器）也当成触屏 | 2026-08-17 |
...
| KD-19 | **屏幕上的唯一性由浏览器断言，不由服务端推断**：quote 携带客户端实测的 `renderedOccurrences`，admission 要求恰好为 1（缺失即拒）；同时 UI chrome 子树标 `data-quote-exclude`，选区相交即不提供引用 | 服务端永远看不见渲染平面——脚注角标、KaTeX 字形、组件 loading 文案（Mermaid）都是渲染器凭空生成的，投影再准也数不到它们。KD-17/KD-18 只能覆盖“源码里有对应物”和“能从源码点名”的构造，Mermaid 这类**组件自渲染 chrome** 两者都不属于（Terra 在 #3757 R5 复现）。唯一能看见那个平面的是浏览器，所以让它出具显式断言、服务端只做限制性消费：断言错只会导致拒绝，永远不会放宽锚定 | 2026-08-17 |

## Tips Contribution（F244）

- 新增一条上下文 tip：从 Thread 头部的「选择消息」入口（或桌面 hover 出的消息级快捷方式）进入多选，可将所选内容导出文本/图片或作为合并卡转发；
