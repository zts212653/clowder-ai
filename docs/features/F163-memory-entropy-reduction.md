...
三猫 + operator在 Harness Engineering 讨论中达成共识：

> **Harness 长期价值 = 对用户决策边界的拟合精度 × 知识压缩后的信噪比**

operator experience（2026-04-15）：
> "我们家其实一直没做的是记忆的熵减。什么东西都越来越多，甚至我发现我们的记忆大多数搜过来置信度都是 mid。"
> "我们得有机制定期审视 harness engineering 在我们家。"

前者（拟合精度）靠operator持续共创，后者（信噪比）靠工程机制。我们前者很强，后者完全缺失。不补，长期乘积趋向平庸——不是因为忘了operator教的东西，而是好的教导被淹没在"大概相关"的噪声里。
...
- **shared-rules 浓缩**：同类规则聚类 → 提议合并 → operator确认 → 浓缩后保留 `source_ids` 可追溯

猫不自主执行合并——产出 pruning 建议，operator拍板。
...
- Review queue：猫只标记和建议，operator 确认 invalidation / archive / merge
...
- [x] AC-C5: operator确认报告的 pruning 建议 actionable（不是无用的噪声）
...
**核心问题**（operator 2026-04-19）：搜"数学之美 圆桌讨论"时最精准命中的 thread 标 `[low]`（因为它是 observed），半相关的 LL-051 标 `[high]`（因为它是 constitutional）。confidence 标签语义错误——反映文档权威性而非搜索匹配质量。
...
**方案**（来源：F169 愿景约束 VAC-C1~C5，opus-47 提出，operator确认加入；Design Gate by gpt52 2026-04-25）：
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 猫不自主删除/合并知识，只产出建议 | 知识是operator思维的结晶，删错了不可逆 | 2026-04-15 |
...
| KD-6 | 晋升四级：observed → candidate → validated → constitutional（最后一级仅 operator） | 保留 observed 隔离态（防偶然偏好过早晋升），砍掉无行为差异的 provisional | 2026-04-16 |
...
| KD-9 | 所有能力必须可开关、可灰度、可 A/B 对比，默认关闭 | operator要求：花里胡哨的功能未必带来提升，必须像 F102 一样可度量可回滚 | 2026-04-16 |
...
- Phase B: operator review（合并/删除知识需要 operator 确认）
- Phase C: operator review（健康报告的 actionability 由operator判断）
