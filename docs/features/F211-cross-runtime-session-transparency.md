...
- Post-close carrier discipline (2026-06-02 cleanup): Bengal/Antigravity review is valuable for lived Desktop constraints, but carrier tooling limits mean it is not treated as the only merge gate. Review packets that ask a carrier cat to inspect F211/F061 surfaces must include exact paths, diff anchors, and short excerpts or accessible artifacts so the review does not depend on `grep`/`sed`, out-of-workspace reads, or full-file reads that may truncate.
- Duplicate carrier status/ack messages are tracked as collaboration harness / carrier UX hygiene, not as an F211 session-transparency acceptance gap. Reopen only with fresh evidence that duplication hides session lifecycle truth or breaks a merge/review route.
