...
| KD-3 | 先修自家 Hub，验证后再修 OfficeClaw | operator拍板 | 2026-04-10 |
| KD-4 | Phase C（OfficeClaw）从 F156 拆出 | operator："和我们家无关是外出务工的事情"，不污染自家 feat 真相源 | 2026-04-10 |

## Known Issue: API 重启后 Session 丢失导致用户惊吓（P1）

> **Reporter**: operator（2026-04-13，从 Claude Code CLI 环境发现）
...
**如果推送到社区（clowder-ai），所有用户都会遭遇同样的惊吓**。社区小伙伴不像operator知道可以查 Redis，他们只会看到"thread 全没了 + 401"→ 以为数据丢失 → 提 issue / 放弃使用。
...
operator实测：刷新页面后 thread 又消失了，只有回到首页再进才恢复。
...
> **来源**：2026-04-14 operator连续反馈。"IM Hub/Signal Hub 打不开"、"创建线程点击无事发生"、"刷新后气泡又跑出来"、"thread 切换仍有约 1s 卡顿"。
>
...
| FU-1 | 精确 IP/域名 allowlist | 独立增强，operator确认需要时立项 |
| FU-2 | 开源社区 setup 文档（手机/Tailscale 章节） | 独立文档任务，operator确认需要时执行 |
