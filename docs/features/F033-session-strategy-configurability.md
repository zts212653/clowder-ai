...
- **2026 follow-up（upstream issue #1329）**: Session State/Chain 变为始终可见的独立状态层；`handoff`、`compress`、`hybrid` 只保存 operator 意图；managed invocation 依据当次可证明的 capability 单独投影 `active`、`degraded` 或 `unavailable`，不得静默改写策略。策略修改从下一次 invocation 起作用于当前 active session。

## User Journey

Scope unit 是一个 user × cat × thread 的当前 active session。

1. 用户首次调用 member 时，即使 provider 从不发 `session_init`，Session Chain 也出现一个逻辑节点；后续 runtime ID 只绑定到该节点。
2. 用户在 Hub 选择任一策略并保存。选择始终保留，界面另行显示本次执行能力及缺失原因，不隐藏选项、不替换策略。
...
- Issue #1329 follow-up：state / policy intent / execution capability 三层解耦；首次 managed invocation 建立逻辑节点；policy revision、execution status 与 hybrid epoch 可审计。
- 遗留项：TD094（压缩效率检测）、TD095（MEMORY.md auto-dump）

## Dependencies
- **Related**: F065（continuity bootstrap）、F211（cross-runtime ownership）、F225（handoff UX）
- **Upstream boundary**: #1329 follows #1208/#1209 but does not backfill new session-policy semantics into #1209.
...
- 2026-02-26：F033 建立 per-cat session strategy 配置契约。
- 2026-03-04：Phase 1–3 完成。
- 2026-08-11：upstream issue #1329 Design Gate / maintainer contract gate 通过，完成三层解耦实现与迁移验证。
