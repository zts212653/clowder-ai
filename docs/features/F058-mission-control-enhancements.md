...
F049 MVP 让operator有了一个任务指挥中心，但实际使用中暴露了三个 bug 和五个增强点。这个 Feature 把它们打包解决。
...
| 依赖关系 | Feature 文档有但不展示 | operator看不到 feature 间的依赖/演化关系 | 🟡 中 |
...
### Phase D：导入状态映射 + Layout 修复（operator实测发现的 bug）

> 2026-03-05 operator实测截图暴露两个 Phase A 遗漏 bug。Phase A～C 代码审查全绿、remote review 全通过，但Ragdoll"愿景守护"只 grep 了代码就打勾，没有实际验证产品效果。operator experience："明明不能用！刷新之后都进度不对吧？右下角那些东西看都看不到！你还不能 done"。
...
> 2026-03-10 operator看完两个 UX 方案后指出："行内展开为什么不能展开完整的？Phase 还能展开子项！" 然后追加要求：统一 feature doc 格式 + 建模板 + 历史迁移。
...
**UX 方案**（operator拍板）：行内多级展开，不需要独立详情页。
...
**决策**：在 I1 模板经operator确认后，做一次批量迁移——把历史 feature docs 的关键字段补齐（frontmatter + Dependencies + AC 格式统一）。

**范围**：只补 parser 需要的结构化字段，不重写内容。

### Phase J：依赖全景 DAG 拓扑图（KD-2 推翻，operator要求）

> 2026-03-10 operator实测 Mission Hub 截图后指出："依赖全景 tab 不是愿景里的样子！"当前实现是平铺卡片网格 + 文本列表，不是真正的 DAG 有向图。Phase H 设计稿评审已经要求 DAG 拓扑（KD-4/KD-5），但实现时没有跟上。
...
| ~~依赖关系可视化图谱~~ | ✅ Phase J | KD-2 已被 KD-4/KD-5 推翻：operator要求 DAG 拓扑，标签展示不够 |
...
- [x] AC-I1: `cat-cafe-skills/refs/feature-doc-template.md` 存在且经operator确认
...
- [ ] AC-I6: 历史 feature docs 批量迁移完成（parser 需要的字段补齐）— 遗留，operator确认可延后
...
| R16 | "先画出设计图我看看" | AC-H3 | Pencil 设计稿 + operator确认 | [x] |
...
| KD-2 | ~~依赖展示用标签而非图谱~~ → **Phase J 推翻** | operator实测后要求 DAG 拓扑（KD-4/KD-5），标签不够 | 2026-03-04 → 2026-03-10 |
| KD-3 | `done` 作为第五个 BacklogStatus | 最小改动，符合现有状态机模式 | 2026-03-04 |
| KD-4 | 从"无 Tab"回退到"两 Tab"（功能列表 + 依赖全景） | 操作型和理解型是本质不同的视角 | 2026-03-06 |
| KD-5 | 依赖全景节点禁止突破屏幕宽度 | operator明确要求，用拓扑布局自适应 | 2026-03-06 |
| KD-6 | Feature doc 标准模板 + 未来立项自动使用 | Progress Dashboard parser 需要统一格式才能可靠提取 | 2026-03-10 |
| KD-7 | Progress Dashboard 用行内多级展开，不做独立详情页 | operator："行内展开为什么不能展开完整的？Phase 还能展开子项！"——两种方案不冲突 | 2026-03-10 |
...
- 右侧操作面板只在选中时出现
- 🤔 不确定搜索框是否太重（operator可能习惯点击不习惯打字）
...
- **核心问题**：operator按 Feature 想还是按任务状态想？答案是 Feature。
...
| 维度 | 四猫分歧 | 最终决策 | 理由 |
|------|---------|---------|------|
| 导航方式 | 2Tab / 搜索 / 3模式 / 密度分层 | **两 Tab**（功能列表 + 依赖全景） | 原决策"无 Tab"，operator提出依赖全景需求后回退（KD-4） |
| 主轴 | 状态列 / 搜索 / 场景 / Feature 行 | **Feature 行** | operator心智模型是 Feature 级别不是 task 级别 |
| Done 处理 | chip / 搜索 / 独立 / 折叠 | **自然沉底 + 折叠** | 不占主画布但可达 |
| 搜索 | 主入口 / 不需要 | **顶部筛选框**（辅助） | 吸收 opus-45 思路但不作为主入口 |
| 返回按钮 | 固定返回 / | **返回之前的 thread** | operator明确要求 |

### Phase H 额外需求（operator补充）
...
#### 设计迭代：加 Tab 切换
原四猫收敛决策是"无 Tab"，但operator提出依赖关系全景需求后，回退为 **两 Tab 方案**：
