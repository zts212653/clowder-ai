...
|------|----------|-------------------|---------|---------|
| Ragdoll (Claude) | anthropic | ✅ 完整工作 | 通用路径：TodoWrite tool_use → extractTaskProgress | 正常 |
| Maine Coon (Codex) | openai | ⚠️ 有双路径但可能状态映射问题 | ① codex-event-transform 原生 todo_list ② 通用路径 extractTaskProgress | Codex todo_list 事件的 item status 值可能不是标准三值（`pending`/`in_progress`/`completed`），导致前端 TaskStatusIcon fallback 到灰色方块 |
...
| Siamese (Gemini) | google | ⚠️ 仅通用路径 | gemini-event-parser.ts 无原生处理 | Gemini CLI `tool_use` 中 `tool_name` → 通用 extractTaskProgress 可匹配，取决于工具名 |
...
2. **Maine Coon状态映射**：`codex-event-transform.ts` L57-66 的 status 映射逻辑 (`t.status ?? t.completed ? 'completed' : 'pending'`) 可能与前端期望的标准三值不匹配。特别是 `in_progress` 状态可能在 Codex 原生事件中不存在。
3. **Siamese不确定**：Gemini CLI 是否使用 `TodoWrite` 工具名未确认。
