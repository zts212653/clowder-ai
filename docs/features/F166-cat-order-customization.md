...
operator experience："总揽这里你这只 47 在太下面了！我希望把你拉到最上面！"

当前猫猫顺序完全由 `cat-template.json` 的 `roster` 字段声明顺序决定，前后端均不排序。operator无法按自己的使用习惯调整猫猫展示顺序。@ picker 和总揽页面都受影响。
...
| KD-2 | 整张列表自由排序，非 pin-top 机制 | operator明确要"拖到最上面"的自由度 | 2026-04-17 |
