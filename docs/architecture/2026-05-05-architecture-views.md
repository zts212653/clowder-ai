...
- 协作漏斗 → 折叠进 Hero 图（operator位置高低暗示漏斗层级）
...
**立意**：AI 团队不是按岗位分工的工具集合，是有名字、有性格、长期协作的队友。每只猫 = 不同引擎 × 同一套 harness，看同一件事会发现不同问题——跨厂商多样性是结构性质量来源，不是职能分工。operator不是 oncall，是 operator——愿景层深度参与，执行层完全放手。
...
**立意**：全局图不是产品 Hero，也不是 runtime stack。它是一张“唯一看全貌”的技术索引图：operator、产品面、协作协议、统一执行平面、共享状态、工具运行时、治理闭环如何咬合。

**画法**：刷新旧 7 层图，保持纵向结构，但把 2026-05 的新组件补进去。

```
┌─ 1. operator / Human Direction Layer ─────────────────────────────┐
...
│  质量多样性：跨族 review · 愿景守护 · operator 终裁                │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
...
三条主链：
1. 任务链：operator 目标 → 球权 → 执行平面 → 工具 → 可见产出
...
## 七、operator决策（2026-05-06 拍板）

1. **图 1 Hero 风格** → **猫咖实景剖面图**（三猫全票 + operator确认）
