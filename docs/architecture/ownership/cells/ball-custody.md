...
summary: 球权事件流（append-only canonical）、BallCustodyProjection 投影/7 态状态机、blocked/dead/void/parked/zombie 形态结构化判定、best-effort 唤醒（ProbeScheduler/WakeSender，Phase B 后续 Task）。F254 在同一责任边界内新增 side-effect freshness 的 seenCursor 命名空间与独立 freshness attention event log，用于判断猫发消息/退球前是否忽略了新消息。值班简报横切读 projection 替代启发式扫描。
canonical_features: [F233, F254]
code_anchors:
...
Watch for new `BallCustodyEvent`, `BallCustodyProjection`, `ballcustody:events:`, `ballcustody:projection:`, `blockedSinceAt`, `ProbeScheduler`, `WakeSender`, `FreshnessAttentionEvent`, `FreshnessAttentionEventLog`, `FreshnessInvocationStateStore`, `FreshnessNoticeService`, `FreshnessReinvokeDecider`, `seenCursor`, or `ball-custody` projector / state-machine code.
