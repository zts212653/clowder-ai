...
summary: 球权与显式等待责任边界：BallCustodyProjection、F254 closure/supplement、F167 action-successor 单账本与 hold rescue authorization，以及 F280 typed AwaitState lifecycle / owner fence / expiry / one-shot consume / canonical user-cancel termination event / immutable continuation carrier。普通 A2A 与 managed-hold 消息投递归 F117 source/Queue/response lifecycle，不再由 Ball disposition 或 Queue receipt 裁决。责任对象各自持久化 origin/frontier/holder/generation/final；attention、carrier success、feedback why 与 source collection 都不裁决 action success 或 wake。
...
code_anchors:
  ...
  - packages/api/src/domains/cats/services/freshness/closure/FreshnessClosureStateMachine.ts
  - packages/api/src/domains/cats/services/freshness/closure/FreshnessClosureStore.ts
  - packages/api/src/domains/cats/services/freshness/closure/RedisFreshnessClosureStore.ts
  - packages/api/src/domains/cats/services/freshness/closure/FreshnessClosureLegacyMigrationState.ts
  - packages/api/src/domains/cats/services/freshness/closure/freshness-closure-store-types.ts
...
- Completed output 必须先成为 MessageStore truth。普通 queued user message 继续由 F117 Queue 的 pending `targets[]` 单一拥有，禁止复制到 supplement；actual dispatch / terminal 分别归 MessageStore dispatchRefs 与 exact response / structured owner。只有 non-Queue relevant unseen 才能通过 distinct `FreshnessSupplementAggregate` 追加责任，且不能复用 closure final key、删除原文或把 supplement 当 replacement。
...
- Queue restart 只从 schema v2 ledger 恢复 source order、pending `targets[]` 与 bounded claim；不得从 invocation success、aggregate parent、target membership 或旧 receipt 推导 Queue membership。实际 dispatch 与 terminal 只能从各自 canonical owner 恢复。
