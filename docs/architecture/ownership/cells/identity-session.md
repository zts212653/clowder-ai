...
static_scan_hints: [catId, AgentRegistry, cat-config, roster, ConnectorThreadBindingStore, bubbleIdentity, session, SessionChainStore, cliSessionId, cascadeId, runtimeSession, continuationOrigin, recoveryDelivery, openedByInvocationId, capsule, "private/profile"]
cited_by:
  ...
  - {feature: F192, date: 2026-07-16, delta: "Phase I — immutable creation-time SessionRecord transition lineage and provider-boundary recovery delivery receipt; harness-eval consumes this truth one-way through a bounded derived projection"}
---
...
- `identity-runtime-session`: F211 owns runtime session identity and binding for long-lived or external runtimes: cascade/conversation IDs, SessionChainStore bridge records, lifecycle registration, hidden external-runtime anchor threads, seal reason, per-session identity history, and immutable source→target continuation provenance stamped at the first provider dispatch.
...
- Changing how a fresh target session records `openedByInvocationId`, `continuationOrigin`, or the metadata-only `recoveryDelivery` receipt.
...
- Stamp continuation lineage only in the atomic SessionChain create path at first `session_init`; later retries, resumes, and generic updates must preserve it unchanged. Eval consumers may scan bounded windows but may not write inferred lineage back.
...
- `identity-runtime-session` is not `harness-eval`. This cell owns source/target lineage and delivery facts; `harness-eval` owns derived trials, semantic assessments, grading, and verdict artifacts. A diagnostic `seq + 1` candidate must never become identity truth.
