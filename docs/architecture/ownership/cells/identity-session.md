...
static_scan_hints: [catId, AgentRegistry, cat-config, roster, ConnectorThreadBindingStore, bubbleIdentity, session, SessionChainStore, cliSessionId, cascadeId, runtimeSession, continuedFromSessionId, openedByInvocationId, capsule, "private/profile"]
cited_by:
  ...
  - {feature: F192, date: 2026-07-17, delta: "Phase I contraction — target SessionRecord stores only immutable openedByInvocationId + continuedFromSessionId; deterministic delivery/transition guarantees stay in runtime contract tests, while harness-eval consumes observed targets for semantic recovery assessment"}
---
...
- `identity-runtime-session`: F211 owns runtime session identity and binding for long-lived or external runtimes: cascade/conversation IDs, SessionChainStore bridge records, lifecycle registration, hidden external-runtime anchor threads, seal reason, per-session identity history, and the immutable source Session backlink stamped when a target Session is created by its first `session_init`.
...
- Changing how a fresh target session records `openedByInvocationId` or `continuedFromSessionId`.
...
- Stamp `openedByInvocationId` in every atomic SessionChain create path. Stamp `continuedFromSessionId` only on the first target created from an explicit `SessionBootstrap` source carried across invocations; same-invocation self-heal/runtime replacement Sessions do not receive that backlink. Generic updates never mutate either creation fact. Eval consumers may scan bounded observed-target windows but may not infer or write lineage.
...
- `identity-runtime-session` is not `harness-eval`. This cell owns the target's minimal causal backlink and opening invocation; `harness-eval` owns derived trials, semantic assessments, grading, and verdict artifacts. Prompt delivery receipts, transition attempts, and `seq + 1` inference are not identity truth.
...
Watch for new or renamed `catId`, `CatId`, `AgentRegistry`, `cat-config`, `roster`, `ReviewerMatcher`, `ConnectorThreadBindingStore`, `Binding`, `bubbleIdentity`, `canonicalInvocationId`, `session`, `SessionChainStore`, `cliSessionId`, `cascadeId`, `runtimeSession`, `continuedFromSessionId`, `openedByInvocationId`, and agent-facing `entity_id` code.
