...
事务必须同时验证 exact entry/target、从 `targets[]` 移除该 pending target、确定 `inputMessageId`、写一条 `DeliveryFailureResult`，以及提交 source owner 已经返回的 failure-evidence private input / internal diagnostic（若有）。若失败的是已有 Agent History message 的 `message_wake`，失败结果直接引用原消息；此前没有 actual dispatch，所以不存在需要“推进”的中间 ref。正常 drain 的 `expectedSelection` 要求它仍是 head；显式操作则以实时 Queue/History join 跳过已经投递的 target，不能借失败路径重复处理另一个已赢得 cutover 的 target：
...
| A43 | 用户查看一条 `@B @C` source，B/C 都仍 pending，并分别选择 B=interrupt、C=guide；确认前 B 已被未读接管，用户另选 D 并取消 C | 服务端重读 live Queue+History，跳过已投递的 B；取消 C 只从 pending set 删除，D 在同一 source entry mutation 中加入并投递。B 的既有 dispatch 不撤销；各 target provider outcome 独立 |
