...
description: "定义消息写入 Chat History、进入有序 Queue、事件驱动调度、运行气泡、Agent 未读上下文与终局收敛的端到端生命周期。"
...
> 消息在共同时间线中的位置何时确定、何时进入有序 Queue、队首何时必然被调度、输入如何交给 client、回复如何原位终局，以及失败或重启后用户看到什么。

### 1.1 设计目标

1. 前端流式气泡、最终 Chat History 与 Agent 上下文使用同一顺序；
...
| 对象 | 持久化 | 唯一职责 |
|---|---:|---|
| Queue Entry | 是 | 保存一个有序的 `messageId` 引用；仍在 Queue 就表示尚未完成正常 dispatch |
| Chat History Message | 是 | 保存输入、Agent 消息与响应气泡；拥有全 thread 的固定 `orderKey` |
| Active Run | 否 | 表示 target 当前正在处理哪些 Queue Entry，并关联唯一响应气泡 |

client adapter 是执行边界，不是第四本账。Scheduler 只是驱动器，不拥有另一份 lifecycle 状态。

```mermaid
flowchart LR
    I["用户 · Connector · Agent · System"] --> H["Chat History<br/>固定 messageId + orderKey"]
    H -->|"需要成员处理"| Q["Durable Queue<br/>有序 messageId 引用"]
...
- Queue membership 是 `queued` 的唯一真相，不在 Message 上复制一个可漂移的 Queue 状态机；
- Active Run presence 是 target occupancy 的唯一真相，不另建 `TargetSlot`；
- 响应气泡的 `processing / completed / failed / canceled / interrupted` 是输出结果，不是 dispatch acceptance 状态。

## 3. 最小数据契约

### 3.1 Queue Entry：只有有序消息引用
...
Queue Entry 不保存 `priority`、attempt、receipt、`DirectPre`、target slot 或运行状态。消息作者、结构化 mention、`producerInvocationId`、可见范围与正文都从它引用的 Chat History Message 读取。

每条输入对应一个独立 Queue Entry：

- 每次用户发送是一条 message + 一条 entry；
- 每次 `post_message` 是一条独立 message；有目标时再建一条 entry；
- 不自动合并连续用户消息；
- 不把多条 wake 合成一条正文；
- 拖动重排或删除 entry 是显式用户操作，修改后的物理顺序立即成为新真相。

### 3.2 Chat History Message：先固定位置，再改变内容
...
type LifecycleMessageMetadata = {
  ...
  visibilityPolicy: 'immediate' | 'after_queue_take' | 'target_only'
}
...
`orderKey` 在消息或响应气泡首次进入 History 时分配，之后永不改变。`completedAt` 只用于耗时与诊断，不能重新排序。

`visibilityPolicy` 是消息的静态发布策略，不是 Queue 状态副本：

- 用户/Connector 输入使用 `after_queue_take`；前端立即可见，但 Queue Entry 仍存在时不能进入 Agent 普通未读上下文；
- Agent 普通输出与 `post_message` 使用 `immediate`；
- 只给某个 Agent 的控制通知使用 `target_only`，仍是普通 History message + Queue Entry，不是另一种 Queue 对象。

### 3.3 Active Run：没有 TargetSlot 与 RunInputRef
...
type ActiveRun = {
  ...
  inputs: readonly QueueEntry[]
  startedAt: number
}
```

`inputs` 直接保存本轮实际取得的不可变 Queue Entry；需要作者或因果信息时由 `messageId` 回读 History。正常 dispatch 在调用 provider 前登记 Active Run，因此同一结构同时承担 admission 与 occupancy，不需要短暂 `TargetSlot`。

provider 只需返回“是否接受”以及 exact execution handle；Start、Append、Steer 是调用方已经知道的动作，不再返回 `started / appended / steered` 这种领域状态。

## 4. 消息入口

### 4.1 用户与 Connector：先进入共同时间线，再入 Queue

```text
收到输入
  → 生成固定 messageId + orderKey
  → 写入 Chat History（前端可见，visibilityPolicy=after_queue_take）
  → 创建 QueueEntry(messageId)
  → Queue commit 后 requestDrain(threadId)
```

消息一发送就出现在前端，因此用户不会看到 Queue row 与最终 History 互相跳位。只要 entry 仍在 Queue，Agent 普通上下文投影就把这条输入视为顺序屏障；到 dispatch admission 时，原子移除 entry 后它才成为 Agent 可读输入。

需要成员处理的 message append 与 Queue enqueue 必须作为一个持久提交完成：不能留下“History 有输入但永远没有 Queue entry”的半提交状态。

### 4.2 Agent 输出与 `post_message`：每次调用都是独立消息

```text
Agent final / post_message
  → 写一条独立 History message（visibilityPolicy=immediate）
  → 若消息含有效结构化目标，创建一个 QueueEntry(messageId)
  → requestDrain(threadId)
```

Agent 消息没有有效目标时只公开给用户，不猜测下一只 Agent。消息若由 live invocation 产生，携带 `producerInvocationId`；它是因果元数据，不是 Queue owner 或递归责任树。

### 4.3 target 在队首执行时解析

入口只保存结构化 mention references，不在 enqueue 时猜默认成员。entry 成为队首后：
...
async function drainExecutableHeads(threadId: string): Promise<void> {
  while (true) {
    ...
    const message = await history.require(head.messageId)
    const targets = await resolveTargetsAtHead(threadId, message)

    if (!targets) return // targetless 且仍有 Active Run
    if (targets.some(target => activeRuns.has(threadId, target))) return

    const admitted = await admitExactHead(head, message, targets)
...
一条 `@B @C` 仍是一个 Queue Entry 和一条输入 message。只有 B、C 都空闲时才 admission；然后分别创建 B、C 的 Active Run 与响应气泡，再并发调用 provider。
...
2. 为每个 target 生成 `invocationId + responseMessageId + startedAt/orderKey`；
3. 持久化尚未激活的 exact callback principal；principal mint/persist 失败时 entry 保持 Queue 中，不能报告 accepted；
4. 原子移除 Queue Entry、打开 `after_queue_take` 输入的 Agent 可见性，并为每个 target 写一条 `processing` ResponseBubble；
...
```ts
async function admitExactHead(head, message, targets) {
  const prepared = await prepareAdmissions(targets, head)
  await principals.persistAll(prepared)

  const taken = await stores.takeHeadAndCreateResponseBubbles({
...
所有会启动运行的入口都必须复用该 cutover。没有 target slot、reservation object 或三态 `DispatchAcceptance`。
...
async function onRunTerminal(run, terminal): Promise<void> {
  ...
  if (terminal.kind === 'failed') {
    await enqueueFailureNoticeForAgentAuthors(run.inputs, terminal)
  }
...
这保留直接失败回退，却不需要 `DirectPre`、`private_control` Queue kind 或 priority 通道。
...
两类可见记录是 cursor barrier：

- Queue 中 `visibilityPolicy=after_queue_take` 的输入；
- `status=processing` 的 response bubble。

Agent 可以在 UI/上下文摘要中知道“成员正在处理”，但持久 cursor 不能越过 processing bubble 并把它当作正文已读。气泡终局后，下一次上下文在原位置读取完整正文，再推进 cursor。

当前被 dispatch 或显式 Append/Steer 的 Queue input 仍作为 exact input 注入 provider，即使普通 cursor 被更早的 barrier 挡住；这不推进 cursor，也不把窗口外消息误标为已读。
...
2. 将当前 Queue Entry 引用的 message 作为必选 exact input；
...
| 位置 | 必须留下的结果 | Queue 行为 |
|---|---|---|
| target 解析后不存在且无 fallback | failure response bubble | entry 被处理，继续下一条 |
| callback principal mint/persist 失败 | 未 accepted；可见诊断 | exact entry 留在 Queue，可重试 |
...
| Queue entry 被取消 | 输入仍留在用户 History，标明未投递/已取消 | 删除 entry，不调用 provider |
...
- Queue take / processing bubble commit 后、provider launch 前退出：同一 bubble 变为 interrupted；
...
```text
History input + Queue 标记 → 消息已发送，等待 dispatch
processing response bubble → exact member 正在处理
terminal response bubble   → 已完成、失败、取消或被重启中断
```

- 输入从发送起就在 Timeline 固定位置，不会 dispatch 后重新插入；
- Queue Panel 是同一 message 的待处理引用，不复制正文事实；
...
| 责任 | 主要代码位置 | 目标改造 |
|---|---|---|
| 输入持久化 | `packages/api/src/routes/messages.ts` | 先写固定 History message，再建最小 QueueEntry；commit 后 requestDrain |
| Queue | `packages/api/src/domains/cats/services/agents/invocation/InvocationQueue.ts` | durable ordered message refs；删除自动 merge、priority 与 attempt/lifecycle 字段 |
...
| Agent 路由 | `packages/api/src/domains/cats/services/agents/routing/AgentRouter.ts` | targetless fallback 从 ingest 后移到 head execution |
| 未读上下文 | `packages/api/src/domains/cats/services/agents/routing/route-helpers.ts` | 复用 delivery cursor、visibility/window 与 projected/exposed ids；支持 barrier + exact input |
| 响应发布 | `packages/api/src/domains/cats/services/agents/routing/route-serial.ts` | admission 创建固定 response bubble；stream/final 原位更新 |
| History 顺序 | `packages/api/src/domains/cats/services/stores/redis/redis-message-append.ts`<br/>`packages/api/src/domains/cats/services/stores/redis/RedisMessageStore.ts` | 首次写入即分配 orderKey/visibility position；更新不重排 |
| Active Runs | `packages/api/src/domains/cats/services/agents/invocation/InvocationTracker.ts` | 只保存 exact run + responseMessageId + QueueEntry inputs；删除 TargetSlot |
| Agent wake | `packages/api/src/routes/callback-a2a-trigger.ts` | 每条 post_message 独立写 History；有 target 才建 entry |
...
| UI | `packages/web/src/components/QueuePanel.tsx` 及消息气泡 | Queue row 引用同一 History message；stream/final 同一气泡；显示 exact Active Run controls |
...
2. 把 QueueEntry 收敛为 ordered message reference，删除自动 merge 与 priority；
3. 调整入口为 History message + Queue reference，并把 target fallback 后移；
4. 建立 admission transaction：take exact head + processing bubble + callback principal；
5. 用 ActiveRun 取代 TargetSlot/RunInputRef/DispatchAcceptance；
6. 把所有调度触发收敛到 `requestDrain`，删除 timer 型正确性兜底；
7. 接通现有未读 cursor 的 queued/processing barrier 与 exact input；
...
迁移不得删除现有 Chat History。旧 Queue 数据只转换为 message reference；不能可靠转换的运行投影保留诊断但不恢复成 Active Run。
...
| 前端 A→B，Agent 却读成 B→A | UI 按 startedAt，visibility 在 final append 时才分配 | message/bubble 首次出现即固定 orderKey；processing 是 cursor barrier；final 原位更新 |
...
| targetless 消息错误投给忙碌成员 | ingest 时过早猜 fallback | entry 到队首且 thread idle 后才选择最近完成成员/default |
...
| ID | 场景 | 必须满足 |
|---|---|---|
| A1 | 用户发送 `@B`，B 繁忙 | 输入立即固定在 History/Timeline；entry 留在 Queue；Agent 普通上下文不可越过 |
...
| A6 | 一条消息 `@B @C` | 一个 entry；B/C 全空闲才 admission；两个独立 response bubble |
...
| A18 | admission 前 principal persist 失败 | entry 仍在 Queue；没有 client side effect；不报告 accepted |
| A19 | crash after Queue take before provider launch | 已有 processing bubble；startup 原位 interrupted；不重放 |
...
- History message 已提交但需要处理时没有对应 Queue Entry；
...
- 为修复上述问题重新增加 TargetSlot、RunInputRef、DispatchAcceptance、receipt ledger 或 timer 型正确性 fallback。

最终判断标准不是“覆盖了多少状态”，而是：普通读者沿一条 message 从固定时间线位置走到一个 terminal bubble 时，每一步只有一个 owner、一个顺序和一个下一触发。
