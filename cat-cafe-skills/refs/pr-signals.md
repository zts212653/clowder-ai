...
- **注册那一刻不做同步探测**：回合只由归一化流里、游标之后、**你自己发出**的召唤评论开启。
  刚发完召唤就想结束本轮的猫，等下一个轮询周期即可
- **别人的召唤不给你开回合**：维护者自己 `@codex review`，你收到的是一条普通 bot 对话通知，
  不会在三十分钟后收到一条你从没发起过的"这轮没回来"
- **注册 ≠ 出口凭据**：回合归 tracker owner，不归某一次 invocation，所以它回答不了
  "事件会回到这一次 invocation 吗"。routing guard 不从 tracking 取凭据（F280 §4b），
  本轮要停下就结清自己手上的 hold（`cat_cafe_complete_managed_hold`）
