...
准备作运行版本、进程或部署归因时，先核对：
1. 监听进程与启动时间；
2. 运行 checkout/commit 与目标 commit；
3. 对应进程日志中的实际行为。

证据缺失时，该项归因保持未知；不依赖这些事实的源码阅读、测试复现和其他调查继续进行。缺少字段本身不能证明 runtime 有问题，也不能把能够自行完成的查询转交 operator。具体调查入口见 `debugging` skill 的“Runtime / 版本归因所需证据”。
