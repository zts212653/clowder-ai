...
2. **等外部条件**（按 2a/2b 判断行动）。**外部条件 = 不在 cat-cafe roster 的实体**：云端 codex (`chatgpt-codex-connector[bot]`) / GitHub bot / PR check / CI / 长 build / 外部 webhook / API 响应。CLI 要退出但还需继续也走这条。**严禁**把外部 identity 投射成本地近似 proxy（例："球权在云端 codex"→ 不可 @ 本地同族猫的任何 variant）
   ...
   - **切换点**：轮询唤醒时发现 Codex connector 的 EYES > 0 → 停止续约，注册 `cat_cafe_register_pr_tracking(repoFullName, prNumber, nextStep=...)`（**只传 repo + PR 号**，`nextStep` 可选且仅供展示），**再调 `cat_cafe_complete_managed_hold`** 结清你手上的 hold。注册只切换投递通道，**它不是出口凭据**——tracking 状态归 tracker owner，不归某一次 invocation（F280 §4b），routing guard 不从那里取凭据。结清 hold 才是你自己的 coordination 动作，也是本轮合法停下的方式；不结清就直接停，会按掉球处理
