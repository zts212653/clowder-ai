...
落点规则约束新产物；发现既有不合规工件不等于取得处置授权。处理范围与当前任务归属按下节执行。

## 执行位置

- `quality-gate`：检查根目录媒体/设计工件（含已跟踪和未跟踪），先核对归属。本次交付引入的命中即 BLOCK 并按上述目录归档；与本次无关的既有工件仅记录，不使当前任务转为清理工作，也不授权移动或删除其他工作的文件。
