...
**Feature vs Tech Debt**：team lead能感知变化 → Feature；只有开发者知道 → Tech Debt。不确定先记 TD。
...
**触发**：team lead说"新功能"/"立项"、讨论收敛确认要做。**不触发**：还在探索 → `collaborative-thinking` Mode A；小修补 → TD。

### Step 0a: 分支检查（铁律，不可跳过）🔴

任何改动前先确认当前分支，**绝对不能在 `main`、`runtime/*` 等保护分支上直接开发**：

```bash
BRANCH=$(git symbolic-ref --short HEAD)
echo "当前分支: $BRANCH"
# runtime/* → 🔴 停！这是生产分支，先切到 feature branch
# main → 🔴 停！先创建 feature branch: git checkout -b feat/xxx origin/main
# feat/* / docs/* / fix/* → ✅ 可以继续
```

违反此规则 = runtime 服务中断（F113 事故教训，见 issue #48）。

### Step 0b: 关联检测（内部 + 社区 issue 都必须做）🔴

**分配 F 编号前，先跑关联检测**，防止重复立项或把子任务误立为独立 feature：

1. **扫描 ROADMAP + features/**：`grep -i "{关键词}" docs/ROADMAP.md docs/features/*.md`
2. **判定**：
...
**检查**：分支非 main/runtime/* ✓ 聚合文件创建 ✓ frontmatter 完整 ✓ ROADMAP 索引 ✓ 关联文档双向链接 ✓ 已 commit ✓

## 讨论 (Discussion)

**两种模式**：

- **采访式（默认）**：team lead口述 → 一次一问澄清（"为什么要？现在怎么做？做完后怎么用？"）→ 排优先级 → 记开放问题。**Anti-anchor**：先让team lead表达完，再分析。

- **开放讨论**：多猫协作。结构：背景 + 我的分析（仅供参考，**先自己想再看**）+ 开放问题（按角色分组）+ 我的倾向（透明推理链）。明确标"这是讨论不是任务"，保护观点独立性。

**讨论结束必须做**：
1. 落盘到 *(internal reference removed)*（含team experience、决策过程、优先级排序）
...
| 类型 | 判断标准 | 确认人 | 方式 |
|------|---------|--------|------|
| **前端 UI/UX** | 用户能看到的改动 | **team lead** | wireframe → team lead OK 后继续 |
| **纯后端** | API/数据模型/内部逻辑 | **其他猫猫** | `collaborative-thinking` 讨论达成共识 |
| **架构级** | 跨模块、新基础设施 | **猫猫讨论 → team lead拍板** | 先出方案再上报 |
...
**流程**：
1. 判断功能类型 → 选择确认路径
2. 前端：画 wireframe（Pencil / 文字版 ASCII）→ 发team lead → 等 OK
3. 后端：`collaborative-thinking` → 拉相关猫讨论 API 契约/数据模型
4. 架构：猫猫讨论 → 结论给team lead → team lead拍板
5. 确认产出归档 *(internal reference removed)*

## Phase 碰头（大 Feature 专属，3+ Phase）🔴

大 scope feature **每个 Phase merge 后**，主动和team lead碰头（不是问"要不要继续"，是确认方向）：
...
AC 全打勾 ≠ 完成（F041 教训：12 项 AC ✅ 但 UI 不可用）。先读原始 Discussion/Interview，自问三个问题：① team lead最初要解决的核心问题？② 交付物解决了吗？③ team lead用这个功能体验如何？

**跨猫交叉验证（强制，F073 自动化）**：

自己先完成三问 → **自动 @ 其他猫**请求独立愿景守护（不要等team lead提醒，直接 @）→ 收到结论 → 对齐 → 填签收表（猫猫 / 读了哪些文档 / 三问结论 / 签收）→ 全部对齐后继续 Step 1。
...
| 阶段 | 关键动作 | 文件 |
|------|---------|------|
| Kickoff | 分 ID → 聚合文件 → ROADMAP → 双向链接 | `docs/features/Fxxx.md` |
| Discussion | 采访/开放 → 落盘 → ROADMAP ref | *(internal reference removed)* |
| **Design Gate** | **分流 → 确认（UX→team lead/后端→猫猫/架构→两边）** | *(internal reference removed)* |
| Completion | 愿景对照 → 跨猫验证 → 更新状态 → 移出 ROADMAP | `docs/features/Fxxx.md` |

## Common Mistakes

| 错误 | 正确 |
|------|------|
| 在 main/runtime/* 上直接开发 | Step 0 分支检查，必须在 feature branch 上（F113 事故，#48） |
| 完成后才补聚合文件 | Kickoff 时就建 |
| AC 打勾就标 done，不读原始需求 | Step 0 愿景对照（F041 教训） |
| 自己验完就收尾 | 跨猫交叉验证是强制的 |
| 删了聚合文件 | 只从 ROADMAP 移除，聚合文件永久保留 |
| 不记录演化关系 | Completion Step 3 必须思考 |
| 讨论完不落盘 | 讨论结束写入 *(internal reference removed)* |
| 等team lead手动协调跨猫守护 | 自己 @ 其他猫发起守护（F073） |
| 每步停下来问team lead"可以继续吗？" | 全链路自驱，只在阻塞/close 时通知team lead |
...
| 社区 feature 只在开源仓打标签，ROADMAP 不同步 | ROADMAP.md 必须同步加 Source=community 条目 |
