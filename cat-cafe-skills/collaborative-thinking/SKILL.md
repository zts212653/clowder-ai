...
1. **理解上下文**：先读项目现状（文件、文档、近期 commits）。只询问缺失且会改变方案的关键信息；已有答案直接复用。
2. **探索方案**：提出 2-3 个备选 + tradeoffs，先说推荐和理由。**YAGNI 无情剪枝**——"以后可能需要"的功能先砍。
3. **呈现设计**：按用户需要一次讲清目标、方案和关键取舍。复杂度决定篇幅；不按固定字数反复询问方向。只有新的价值取舍或授权缺口需要用户决定。
4. **产出**：需要且已授权落盘时，将设计保存到 `feature-specs/YYYY-MM-DD-{topic}-design.md`。用户已授权实现就继续；只要求讨论时交付分析。
...
**收敛时按实际产物检查沉淀需要**：下列写入须已在任务授权范围内；只读分析先给建议，讨论本身不授权修改治理文件。
...
**追溯链**：复用当前 issue、thread 或文档来源；确有任务登记与纪要交付时再更新相应链接，不为每次讨论新增 BACKLOG。
