...
| **C 收敛沉淀** | 任何讨论产出了决策/规则/否决理由 | 纯问答（结论在 thread 里已够）、team lead说"不用记" |
...
Phase 3: team lead选扇入者
Phase 4: 扇入综合（会议纪要 + 行动项）
Phase 5: 其他猫审阅补充（纠正误读）
Phase 6: team lead反馈 + 最终确认 → 进入 Mode C
```
...
实现方式：`routeParallel()` 或team lead分别 @ 各猫并强调"先独立思考"。

**Phase 2 触发**：各方基本一致 → 跳过；存在明显分歧 → 需要（限 2-3 轮）；team lead说"够了" → 跳过。

**Phase 4 综合必须包含**：各方观点摘要 / 共识区 / **分歧区**（不要抹平！）/ 待决事项 / 行动项。

**扇入者默认**：Brainstorm 类 → team lead；技术讨论 → 指定综合者 + 指定把关者。team lead可随时覆盖。
