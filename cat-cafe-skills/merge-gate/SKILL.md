...
tips_exempt: F314 extends harness-internal review/source provenance policy; no distinct end-user-invocable capability surface
triggers:
  - "合入 main"
  - "merge"
  - "准备合入"
...
#       )
#     ⚠️ 注册只切换投递通道，**不是本轮的出口凭据**。回合归 tracker owner、不归某一次
#     invocation，所以 routing guard 不从 tracking 取凭据（F280 §4b）。要合法停下，
#     就结清你自己手上的 hold：cat_cafe_complete_managed_hold(disposition='handled')。
#     禁止把任意 active tracker 或一句“PR tracking 会叫我”当出口。
#     bot 回合由轮询按你自己发的召唤评论开启；开了而最终没回来会主动通知你（不再石沉大海）。
