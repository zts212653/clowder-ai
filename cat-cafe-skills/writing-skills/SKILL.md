...
4. **验证**：`pnpm check:skills`、`pnpm check:skills:references`、`pnpm check:skills:surfaces` 全绿；涉及 API route / localhost / script / CLI command / 第一方执行面时，再跑该 claim 对应的定向 checker/test。只有共享契约、门禁执行链或 targeted 无法覆盖跨包合流风险时才跑 full `pnpm gate`；五轴高风险单独加强对应独立审查 / 授权，不把 `pnpm check` 当小改的默认收据。
...
| 小改 skill 直接 push 不跑检查 | 可能把 raw first-party `curl localhost` 主路径带进 main，下一只合 PR 的猫才踩雷 | 跑 `check:skills:references` + `check:skills:surfaces` 与 claim 对应的定向测试；按实际影响范围决定是否升 full gate |
