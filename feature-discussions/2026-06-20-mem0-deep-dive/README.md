...
- **水分与命门**：① benchmark 数字虚高且不可直接复现；② "省 90% token"只算检索侧、藏了提取成本；③ **写入路径每条消息一次 LLM 调用且会出错——一条错记忆比没记忆更糟**；④ 有真实生产翻车案例。
...
> **架构师视角**：云厂商爱 mem0，正因为它是个能让你顺便买它"数据库 + 模型"的**漏斗**。这是"做层"的甜蜜——但也是软肋（见 §5.4）。
> 另一个硬证据：**阿里云落地版是纯向量、没上图库**。这告诉你生产里真正承重的是哪块（见 §5.5）。
...
5. **持久、结构化、跨会话/跨 Agent 的状态**——这是**再长的上下文也给不了**的价值（见 §5.4 的判断）。
...
写入每条消息都要一次 LLM 调用，**且会出错**：抽错、错误合并、甚至幻觉出一条不存在的"事实"。**一条错记忆比没记忆更糟——它会悄悄污染之后所有检索。** 讽刺的是 mem0 主打医疗/心理等高风险垂直，恰恰是最不该信任自动写入的场景。（→ 该由缅因猫专项 review 安全性。）

### 5.4 "省 token"卖点会随长上下文 + prompt 缓存变便宜而贬值 🟡
mem0 真正耐打的价值不是省钱，而是**结构化、可查询、跨会话的持久状态**。判断一个项目要看它哪个价值"长上下文杀不死"——省 token 不是，持久结构化记忆才是。

### 5.5 图库（mem0g）"听起来强"但生产里常被砍 🟡
