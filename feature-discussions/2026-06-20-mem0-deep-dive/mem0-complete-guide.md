...
**图片资产（全 10 张精美图已就位 `assets/`，SVG 矢量源 + 2x PNG）：**
- 第一部分：`mem0-fig1-1-pipelines`（两条管道）、`mem0-fig1-2-pluggable-arch`（可插拔架构）✅ **烁烁已绘制并嵌入**（风格与二、三部分统一）
- 第二部分：`mem0-cloud-positioning` / `mem0-aliyun-arch` / `mem0-volcano-arch` ✅
- 第三部分：`ddm-fig1~5`（契约对撞 / 问题全景 / 抽象层级错配 / blast radius / 错误vs正确）✅
- 原 Mermaid 蓝图均收进各图下方 `<details>` 保留，可编辑。

**待办（下一阶段·烁烁，等宪宪定稿后做以免返工）：**
- **整体排版优化**：三部分之间加视觉分隔；统一图注样式；长表格可做成卡片；TL;DR 可做成视觉摘要页。
- **可补充的图（建议）**：① 全局导览图（三部分逻辑流：原理 → 大厂 → 决策）；② 1.1 的"全记 / mem0 / 全忘"三态对比图。
- 砚砚 review 的 2 处增补（商业软肋 / 多租户隔离安全）待宪宪决定是否并入 §1.5 —— 若并入会影响排版，故排版置后。

*mem0 全景解析 · 2026-06 整合版。架构图为 SVG 矢量源 + PNG；流程图为 Mermaid，可在 GitHub / VS Code / Typora 渲染。*
