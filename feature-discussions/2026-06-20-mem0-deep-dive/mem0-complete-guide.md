...
🎨 **图 3·1（待烁烁出图 · 替换 ddm-fig1）**｜proxy vs DDM vs mem0 三者定位——proxy 是最"哑"的 SQL 流量路由器（只看读/写、不懂数据），DDM 懂分片结构，mem0 必须读懂对话语义；proxy 在"数据访问层最低层"，mem0 在"应用 / 语义层最高层"，中间隔着整个栈。
...
🎨 **图 3·2（待烁烁出图 · 替换 ddm-fig2）**｜两条不相交的通路——上：应用 → proxy → 主 / 从库（**SQL 流量**）；下：AI Agent → mem0 → 向量库（**对话语义**）。两条管道平行、不相交，中间标注三处不同：数据形态 / 触发时机 / 调用方。
...
🎨 **图 3·3（待烁烁出图 · 替换 ddm-fig3）**｜平台化拆成两个子命题——A 多协议数据访问平台 ✅（数据访问层横向扩展，与 AI 无关）；B 集成 mem0 分两路：B1 并列模块 ✅（解耦旁挂）vs B2 嵌入 SQL 转发关键路径 ❌（承重路径上凭空挂 LLM）。
...
🎨 **图 3·4（待烁烁出图 · 替换 ddm-fig4）**｜旁路异步架构——proxy 转发关键路径（应用 → proxy → 主 / 从库）保持纯粹；旁路**镜像**一份 SQL 操作流（虚线、不阻塞）喂给一个**独立 AI 模块**沉淀 SQL 操作记忆。标注铁律：旁路模块挂掉 ↛ 影响转发。
...
🎨 **图 3·5（待烁烁出图 · 替换 ddm-fig5）**｜平台化的正确姿势——左（❌）：往 proxy 承重转发路径上缝 mem0，做成"杂货铺"；右（✅）：proxy 做多协议数据访问平台 + 记忆走 B1 并列模块 / 旁路智能，两条线解耦。
