...
# Probe the model's exact backend import; mirrors whisper-api.py dispatch.
if [ -d "$VENV_DIR" ] && [ -x "$VENV_DIR/bin/python3" ]; then
  _dep_ok=1
  case "$MODEL" in
    *Qwen3-ASR*)
      # Qwen identity requires mlx-audio.
      "$VENV_DIR/bin/python3" -c "import mlx_audio" 2>/dev/null || _dep_ok=0 ;;
    mlx-community/whisper-*)
      # MLX Whisper identity must not silently normalize to faster-whisper.
      "$VENV_DIR/bin/python3" -c "import mlx_whisper" 2>/dev/null || _dep_ok=0 ;;
    *)
      # Short/CTranslate2 model identities require faster-whisper.
      "$VENV_DIR/bin/python3" -c "import faster_whisper" 2>/dev/null || _dep_ok=0 ;;
  esac
  if [ "$_dep_ok" = "0" ]; then
