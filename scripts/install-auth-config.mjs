...
const TEST_SANDBOX_ROOT_ENV = 'CAT_CAFE_TEST_SANDBOX_ROOT';
...
function usage() {
  ...
  node scripts/install-auth-config.mjs test-guard-probe --root DIR
    Test-only, side-effect-free: exits 0 if the sandbox guard would allow that root, 1 if it refuses,
    2 outside a test process. Reads and writes nothing.`);
  process.exit(1);
}
 ...
 * with-test-home.sh to export CAT_CAFE_TEST_SANDBOX left this guard fail-OPEN
 * for a bare `node --test`, which is how account stores got written (P1-5/P1-6).
 * NODE_TEST_CONTEXT is set by the node:test runner itself and is inherited by
 * the installer child, so a test body cannot spawn its way around the boundary.
 */
...
/**
 * The PHYSICAL path of `target`: absolute, every symlink resolved, so two names
 * for the same directory compare equal (P1-10).
 *
 * The previous "realpathSync, fall back to the lexical path on throw" was open
 * exactly where it mattered: an install target usually does NOT exist yet, so
 * the call threw and the SYMLINKED PARENT was never resolved. Walk up to the
 * nearest existing ancestor, canonicalise that, re-append what is missing.
 */
function normaliseRoot(target) {
  ...
  for (;;) {
    try {
      ...
      return pending.length > 0 ? path.resolve(real, ...pending) : real;
    } catch {
...
/**
 * Every home that must stay off-limits to a test process (P1-7).
 *
 * The passwd home is ALWAYS in this set. Returning CAT_CAFE_TEST_REAL_HOME
 * *instead of* it let a spawner un-protect the operator's real home by pointing
 * that variable at a fake path; it is a hint, not the escape hatch, so it may
 * only ADD a protected path. userInfo() reads passwd and ignores $HOME.
 */
function protectedHomes() {
  ...
  return [...new Set(homes.filter(Boolean).map(normaliseRoot))];
}

/** Containment by path segments, not by substring: `/a/bc` is not inside `/a/b`. */
function isAtOrUnder(target, root) {
...
/**
 * Sandbox roots the spawning test declared for itself, as a PATH-style list.
 *
 * The TS twin (packages/api/src/config/test-config-write-guard.ts) can tell an
 * inherited root from a test-chosen one by snapshotting env at module load:
 * roots a test sets itself are assigned later. A standalone installer gets no
 * such seam — it is a fresh child process, so every root arrives at once, and
 * install.sh:635-641 shows the ambiguity is real (it deliberately targets a
 * global root that differs from --project-dir). So the installer inverts the
 * rule: instead of blacklisting roots that look inherited, it writes only where
 * the test positively declared a sandbox. No declaration means refuse.
 */
function declaredSandboxRoots() {
  return (process.env[TEST_SANDBOX_ROOT_ENV] ?? '')
...
/**
 * Defense-in-depth for tests. Duplicated from
 * packages/api/src/config/test-config-write-guard.ts — see that file's header.
 * The two guards enforce the same property, not the same code: this copy adds
 * the sandbox declaration because a child process has no module-load seam.
 */
function assertSafeTestConfigRoot(targetRoot, source) {
  ...
  const resolvedTarget = normaliseRoot(targetRoot);
  const unsafeTargets = [];
  if (resolvedTarget === normaliseRoot(REPO_ROOT)) unsafeTargets.push(`repo root (${REPO_ROOT})`);
  for (const home of protectedHomes()) {
    if (resolvedTarget === home) unsafeTargets.push(`HOME (${home})`);
  }

  // Runtime/workspace roots are exported by the running API process when it
  // launches a cat's shell; a test never sets them for the installer, and
  // with-test-home.sh strips them. Anything still here came from the launcher.
  for (const name of ['CAT_CAFE_RUNTIME_ROOT', 'CAT_CAFE_WORKSPACE_ROOT']) {
    const raw = process.env[name];
    if (!raw?.trim()) continue;
    const inherited = normaliseRoot(raw);
    if (isAtOrUnder(resolvedTarget, inherited)) {
      unsafeTargets.push(`store root inherited from the launching process (${inherited})`);
    }
  }

  const declared = declaredSandboxRoots();
  if (declared.length === 0) {
    unsafeTargets.push(`no declared test sandbox root (set ${TEST_SANDBOX_ROOT_ENV} to the fixture dir)`);
  } else if (!declared.some((root) => isAtOrUnder(resolvedTarget, root))) {
    unsafeTargets.push(`a root outside the declared test sandbox (${declared.join(path.delimiter)})`);
  }
  ...
  throw new Error(
    `[test sandbox] Refusing ${source} write/migration against ${unsafeTargets.join(' / ')}. ` +
      `Point ${TEST_SANDBOX_ROOT_ENV} at the fixture dir this test owns, and run tests through ` +
      'scripts/with-test-home.sh, which strips inherited runtime/workspace/global roots.',
  );
}
...
try {
  ...
  // Test-only, side-effect-free boundary probe. P1-7's regression has to ask
  // "would you refuse the operator's real passwd HOME?" — and every other way
  // of asking lets a BROKEN guard answer by writing into that home, which is
  // exactly what the regression is supposed to prevent. The exit code is the
  // whole answer: 0 = would allow, 1 = refused, 2 = not available here. Nothing
  // is read, written or migrated.
  //
  // P2-11: this used to fire on an ENV VAR, ahead of dispatch, so a production
  // `client-auth set` that merely INHERITED the variable exited 0 having created
  // nothing — a silent success no-op, worse than the failure it replaced. A
  // stray variable must never change what a production command does, so the
  // trigger is now an explicit subcommand AND a genuine test process. Neither
  // alone is enough, and the env var no longer exists.
  if (positionals[0] === 'test-guard-probe') {
    if (!isTestProcess()) {
      console.error('Error: test-guard-probe is only available inside a test process');
      process.exit(2);
    }
    assertSafeTestConfigRoot(getRequired(values, 'root'), 'install-auth-config.guardProbe');
    process.exit(0);
  }
