...
Maintainers apply labels to issues for tracking. **Format rules** (so we don't end up with 100 label variants):

| Label | Format | Color | Example |
|-------|--------|-------|---------|
| Feature tracking | `feature:F{number}` | green `#0E8A16` | `feature:F113`, `feature:F115` |
| Issue type | `bug` / `enhancement` / `question` | default | `bug` |
| Priority | `P0` / `P1` / `P2` | red/orange/yellow | `P1` |
| Status | `wontfix` / `duplicate` / `blocked` | gray | `duplicate` |

**Do**:
- Use `feature:F115` (with `feature:` prefix + F-number)
- One feature label per issue (an issue belongs to one feature)
- Add type labels (`bug`, `enhancement`) alongside feature labels

**Don't**:
- `F115` (missing prefix) — won't render with correct color/grouping
- `feat-F115`, `f115`, `Feature F115` — inconsistent casing/format
- Invent new label categories without checking existing ones first

> Contributors: you don't need to add labels — maintainers will tag your issue after triage.
...
Maintainer 会给 issue 打标签。**命名规范**（避免出现一百种标签格式）：

| 标签 | 格式 | 颜色 | 示例 |
|------|------|------|------|
| 功能追踪 | `feature:F{编号}` | 绿色 `#0E8A16` | `feature:F113`、`feature:F115` |
| Issue 类型 | `bug` / `enhancement` / `question` | 默认 | `bug` |
| 优先级 | `P0` / `P1` / `P2` | 红/橙/黄 | `P1` |
| 状态 | `wontfix` / `duplicate` / `blocked` | 灰色 | `duplicate` |

**正确**：`feature:F115`（带 `feature:` 前缀 + F 编号）
**错误**：`F115`、`feat-F115`、`f115`、`Feature F115`

> 贡献者不需要自己打标签——maintainer 会在分类时标注。
