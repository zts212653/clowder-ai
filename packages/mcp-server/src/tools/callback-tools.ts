...
export const callbackTools = [
  ...
  defineTool({
    ...
    description:
      ...
      'GOTCHA: max 3 TIMER holds (wakeAfterMs) per (thread, cat) within a sliding ~1h window (anchored to the last successful timer hold) — the 4th timer hold returns 429 with retryAt/retryAfterMs, you MUST pass (@ another cat or @co-creator). wakeWhen managed commands are self-grounded and neither consume nor are blocked by window slots (#1471). ' +
