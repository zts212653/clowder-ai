...
describe('cat_cafe_community_guardian_signoff MCP tool', () => {
  let originalEnv;
  let originalFetch;
  let tempDir;
  ...
  beforeEach(() => {
    ...
    tempDir = join(tmpdir(), `cat-cafe-guardian-signoff-${Date.now()}-${Math.random()}`);
  ...
  test('uses the refreshed credential file and keeps credentials out of the body', async () => {
    ...
    writeFileSync(credentialPath, JSON.stringify({ invocationId: 'inv-guardian', callbackToken: 'tok-guardian' }), {
      mode: 0o600,
    });
    ...
    globalThis.fetch = async (url, options) => {
      ...
      return { ok: true, json: async () => ({ guardianAssignment: { signedOff: true, approved: true } }) };
    };

    const { handleCommunityGuardianSignoff } = await import('../dist/tools/callback-tools.js');
    const checklist = [
      {
        id: 'vision-alignment',
        label: 'Vision alignment',
        required: true,
        evidence: 'PR body and issue scope match',
        verifiedAt: 123,
        verifiedBy: 'codex',
      },
    ];
    const result = await handleCommunityGuardianSignoff({
      caseId: 'case-42',
    ...
    assert.ok(capturedUrl.endsWith('/api/community-issues/case-42/guardian-signoff'));
    assert.equal(capturedOptions.headers['x-invocation-id'], 'inv-guardian');
    assert.equal(capturedOptions.headers['x-callback-token'], 'tok-guardian');
    const body = JSON.parse(capturedOptions.body);
    assert.deepEqual(body, { signoffToken: 'signoff-token', checklist, approved: true });
  ...
  test('supports the persistent agent-key carrier', async () => {
    ...
    globalThis.fetch = async (_url, options) => {
      capturedOptions = options;
      return { ok: true, json: async () => ({ guardianAssignment: { signedOff: true, approved: false } }) };
    };

    const { handleCommunityGuardianSignoff } = await import('../dist/tools/callback-tools.js');
    await handleCommunityGuardianSignoff({
      caseId: 'case-43',
      signoffToken: 'signoff-token',
      checklist: [],
      approved: false,
  ...
  test('is registered with a schema and handler', async () => {
    ...
    const tool = callbackTools.find((entry) => entry.name === 'cat_cafe_community_guardian_signoff');
    assert.ok(tool, 'cat_cafe_community_guardian_signoff must be registered');
