 ...
 *
 * 描述 skill 应该 symlink-mount 到哪些 provider 目录。
 * 替代 skill-mount.ts 中硬编码的 4-provider 假设
 * （claude/codex/gemini/kimi），让未来通过 ACP/A2A 接入的 client
 * 也能通过自定义路径挂载。
 */

/** Built-in standard provider — fixed enum of known clients. */
export type StandardProviderId = 'claude' | 'codex' | 'gemini' | 'kimi';

/** Ordered list of standard providers (canonical iteration order). */
export const STANDARD_PROVIDER_IDS: readonly StandardProviderId[] = ['claude', 'codex', 'gemini', 'kimi'] as const;

/** Mount config for a standard provider. */
export interface StandardProviderRule {
  /** Whether to mount skills into this provider's directory. */
  enabled: boolean;
  /** Skills directory path relative to projectRoot (e.g. ".claude/skills"). */
  path: string;
}

/** Custom mount target — for ACP/A2A or unknown clients. */
export interface CustomProviderRule {
...
export interface MountRules {
  /** Schema version. */
  version: 1;
  /** Standard providers — always contains all 4 entries. */
  providers: Record<StandardProviderId, StandardProviderRule>;
  /** Custom mount paths for ACP/A2A/unknown clients (may be empty). */
  customPaths: CustomProviderRule[];
}
 ...
 * Replaces the provider-keyed MountRules format for persistence;
 * readMountRules() adapter converts back to MountRules for downstream consumers.
 */
export interface MountRuleEntry {
  /** Provider identifier: 'claude' | 'codex' | 'gemini' | 'kimi' (or custom). */
  name: string;
  /** Skills directory path relative to projectRoot (e.g. ".claude/skills"). */
  path: string;
  /** Whether to mount skills into this provider's directory. */
