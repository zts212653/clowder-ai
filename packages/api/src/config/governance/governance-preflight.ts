...
export async function checkGovernancePreflight(
  ...
  // Skills check: governance is initialized when registry + config file pass.
  // Skill deployment (symlinks) is a separate concern handled by drift detection.
  // When all skills are globally disabled, bootstrap legitimately creates zero
  // symlinks — this is NOT a governance failure. Skip this check to avoid
  // infinite bootstrap loops when no skills are enabled.
  return { ready: true };
}
