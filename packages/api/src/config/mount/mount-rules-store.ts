...
function isSafeProviderMountPath(value: string): boolean {
...
function isSafeCustomMountPath(value: string): boolean {
  ...
  return isSafeProviderMountPath(value);
}

function validateStandardProviderRule(input: unknown): StandardProviderRule | null {
  if (!input || typeof input !== 'object') return null;
  const entry = input as Record<string, unknown>;
  if (typeof entry.enabled !== 'boolean') return null;
  if (typeof entry.path !== 'string' || entry.path.length === 0 || !isSafeProviderMountPath(entry.path)) return null;
  return { enabled: entry.enabled, path: entry.path };
}

function validateCustomProviderRule(input: unknown): CustomProviderRule | null {
  if (!input || typeof input !== 'object') return null;
  const entry = input as Record<string, unknown>;
  if (typeof entry.alias !== 'string' || entry.alias.length === 0) return null;
  if (STANDARD_PROVIDER_SET.has(entry.alias)) return null;
 ...
 * Standard providers are mapped to providers record; others go to customPaths.
 * Missing standard providers are filled from DEFAULT_MOUNT_RULES.
 */
export function mountRuleEntriesToMountRules(entries: MountRuleEntry[]): MountRules {
  ...
  const providers: Partial<Record<StandardProviderId, StandardProviderRule>> = {};
  const customPaths: CustomProviderRule[] = [];

  for (const entry of entries) {
    if (standardSet.has(entry.name)) {
      const providerId = entry.name as StandardProviderId;
      const rule = validateStandardProviderRule(entry);
      providers[providerId] = rule ?? { ...DEFAULT_MOUNT_RULES.providers[providerId], enabled: false };
    } else {
      if (entry.enabled !== true) continue;
      const rule = validateCustomProviderRule({ alias: entry.name, path: entry.path });
      if (rule) customPaths.push(rule);
    }
  }
  // Fill missing standard providers from defaults
  for (const id of STANDARD_PROVIDER_IDS) {
    if (!providers[id]) providers[id] = { ...DEFAULT_MOUNT_RULES.providers[id] };
  }
  return { version: 1, providers: providers as Record<StandardProviderId, StandardProviderRule>, customPaths };
}
 ...
 * user-editable and may be hand-broken. All four standard providers must be
...
export function validateMountRules(input: unknown): MountRules | null {
  ...
  const validatedProviders: Partial<Record<StandardProviderId, StandardProviderRule>> = {};
  for (const id of STANDARD_PROVIDER_IDS) {
    const entry = validateStandardProviderRule(providersObj[id]);
    if (!entry) return null;
    validatedProviders[id] = entry;
  }
  ...
  const validatedCustomPaths: CustomProviderRule[] = [];
  const customAliases = new Set<string>();
  for (const cp of customPaths ?? []) {
    const entry = validateCustomProviderRule(cp);
    if (!entry) return null;
  ...
  return {
    version: 1,
    providers: validatedProviders as Record<StandardProviderId, StandardProviderRule>,
