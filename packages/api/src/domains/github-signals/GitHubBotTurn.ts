 ...
 *
 * This is now the ONLY definition of "a trigger". There used to be a second one — a regex inside
 * the registration-time coverage verifier — and #1394 deleted that verifier along with the whole
 * registration probe. Anything that later needs to recognise a summon reads it from here, so the
 * question cannot drift back into two answers.
 */
...
/**
 * A round is OURS or it is nobody's.
 *
 * Opening a round on a stranger's `@codex review` gave the tracking owner a pending state it
 * never created, and thirty minutes later A28 announced that a request the owner never made had
 * gone unanswered. The bot answers on the PR, not to a person, so `close` stays open to any
 * known bot: an answer ends whichever round is actually open, and if none is, it ends nothing.
 *
 * `self` unresolved is NOT `self` — an unattributable comment opens nothing. The cost of that
 * is one missing "it never came back" notice; the cost of the other direction is a timeout
 * reported for a request nobody in this thread ever sent, which is the fabricated-signal class
 * F280 A26 puts above a missing one.
 */
export function classifyBotTurnTransition(input: {
  readonly author: string;
  readonly body: string;
  readonly at: number;
  /** Did OUR OWN GitHub identity write this? Only then may it open a round. */
  ...
  if (input.self !== true) return undefined;
