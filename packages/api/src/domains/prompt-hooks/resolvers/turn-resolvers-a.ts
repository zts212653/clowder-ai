  ...
  resolve(input: AssemblerInput): ResolveResult {
    ...
    const replyInstruction =
      coordination?.phase === 'terminal'
        ...
        : `仅当你是在回复这条入站 coordination 时，才用 cross_post_message(threadId="${sourceThreadId}", targetCats=["${senderCatId}"])。\n若你要在当前 owner thread 发起新的 workflow action，它不是回复：local cat review 使用普通 @reviewer + PR/exact HEAD/accepted source，reviewer 再以普通 @author + typed localReviewVerdict/reviewedHeadSha/reviewSubjectRef/acceptedSourceRef/acceptedRevision 回球；external review 或 implement 可用其结构化 action carrier。其他协调若无专用 carrier，显式传 coordination={phase:"active", subjectRef:"<subjectRef>"}，禁止继承更早的 task source。\n本 thread 的 @${senderCatId} 不会路由回对方（对方 session 在另一个 thread）`;
