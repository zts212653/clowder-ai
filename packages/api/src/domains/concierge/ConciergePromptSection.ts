...
export function buildConciergePromptLines(config: ConciergeConfig, _threadId?: string): string[] {
  ...
  return [
    ...
    '- 跳转引用：逐字复制搜索结果行给出的完整标记，如 `[跳过去 R1｜目标标题｜末尾校验码]`（带用户去对应 thread/message）。',
    '- `Rn｜目标标题｜末尾校验码` 是不可拆分的完整绑定；任一字段不匹配时系统会拒绝按钮。不要只写 Rn，不要自行改标题或校验码，不要手写 threadId/messageId。',
    '- ⚠️ R-handle 仅对当前搜索结果有效——历史消息中的 R1 可能指向不同目标。请只引用本轮搜索结果里的 R-handle，不要复述之前对话中出现过的旧标记。',
    '',
    ...
    '**目标**: [relay/go 必须逐字复制 `Rn｜目标标题｜末尾校验码` 完整绑定；propose_thread/investigate 填查询内容]',
    ...
    '',
    '**目标引用纪律：**',
    '- relay/go 只能逐字复制前面搜索结果里的完整绑定（`Rn｜目标标题｜末尾校验码`），不要只写 Rn，不要自行重写任何字段',
    '- validator 会同时核对 R-handle、标题与校验码；三者不属于同一条 thread 结果就不会渲染确认按钮',
    '',
    '**关键纪律：**',
