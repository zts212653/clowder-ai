...
local retiredTargetIdsRaw = ARGV[6]
local raw = redis.call('HGET', KEYS[1], id)
if not raw then return {-1, ''} end
local row = cjson.decode(raw)
...
if mode == 'processing' then
  ...
  local retiredTargetIds = row.claimedTargetIds
  if retiredTargetIdsRaw and retiredTargetIdsRaw ~= '' then
    retiredTargetIds = cjson.decode(retiredTargetIdsRaw)
  end
  if type(retiredTargetIds) ~= 'table' or #retiredTargetIds == 0 then return {0, raw} end
  local claimed = {}
  for i = 1, #row.claimedTargetIds do claimed[row.claimedTargetIds[i]] = true end
  local retired = {}
  for i = 1, #retiredTargetIds do
    local targetId = retiredTargetIds[i]
    if type(targetId) ~= 'string' or targetId == '' or retired[targetId] or not claimed[targetId] then
      return {0, raw}
    end
    retired[targetId] = true
  end
  attempted.targets = retiredTargetIds
  ...
  for i = 1, #row.targets do
    if not retired[row.targets[i]] then remainingTargets[#remainingTargets + 1] = row.targets[i] end
  end
  local remainingClaimedTargets = {}
  for i = 1, #row.claimedTargetIds do
    if not retired[row.claimedTargetIds[i]] then
      remainingClaimedTargets[#remainingClaimedTargets + 1] = row.claimedTargetIds[i]
    end
  end
  if #remainingTargets == 0 then
    ...
    if #remainingClaimedTargets > 0 then
      row.status = 'claimed'
      row.claimedTargetIds = remainingClaimedTargets
    else
      row.status = 'queued'
      row.claimId = nil
      row.claimedAt = nil
