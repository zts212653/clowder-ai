...
function recordWriteOpportunityPresentation(input: {
  ...
  for (const [outcome, receipts] of [
    ...
    const accepted = input.service.recordPresentation(receipts, {
      outcome,
    ...
    messages.push({
      ...
      content: JSON.stringify({
        ...
        opportunityIds: accepted.map((receipt) => receipt.opportunityId),
      }),
...
function createPresentationDeliveryAttempt(input: {
  ...
  return {
    async confirm(): Promise<AgentMessage[]> {
      ...
      messages.push(
        ...recordWriteOpportunityPresentation({
          catId: input.catId,
          invocationId: input.invocationId,
          promptGenerationId,
          evidenceRef,
...
export async function* invokeSingleCat(deps: InvocationDeps, params: InvocationParams): AsyncIterable<AgentMessage> {
  ...
  const resolveAsrPersonMemoryOpportunities = (continuity: ContextContinuityHandshake | undefined): void => {
    asrPersonMemoryReceipts = [];
    if (!continuity || !params.asrPersonMemoryScenes?.length) return;
    const resolution = asrPersonMemoryPromptService.resolve({
      candidates: params.asrPersonMemoryScenes,
      serverScope: { ownerUserId: userId, threadId, consumerCatId: catId },
      continuity,
      now: Date.now(),
      // The contract trial has no persisted disposition ledger yet. Queue-batch duplicates are
      // still rejected inside resolve(); cross-invocation terminal lineage remains a named Wave 2 predicate.
  ...
  try {
    ...
    resolveAsrPersonMemoryOpportunities(contextContinuityHandshake);
