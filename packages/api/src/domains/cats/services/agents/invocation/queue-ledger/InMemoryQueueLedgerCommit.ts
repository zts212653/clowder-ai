...
function commitClaimedTarget(
  ...
  const claimedTargetSet = new Set(entry.claimedTargetIds);
  const claimedTargets = input.retiredTargetIds ? [...new Set(input.retiredTargetIds)] : [...entry.claimedTargetIds];
  if (
    claimedTargets.length === 0 ||
    claimedTargets.length !== (input.retiredTargetIds?.length ?? claimedTargets.length) ||
    claimedTargets.some((targetId) => !claimedTargetSet.has(targetId))
  ) {
    return { outcome: 'state_changed' };
  }
  ...
  const remainingClaimedTargets = entry.claimedTargetIds.filter((targetId) => !claimedSet.has(targetId));
  if (remainingTargets.length === 0) {
    ...
    if (remainingClaimedTargets.length > 0) {
      remaining.status = 'claimed';
      remaining.claimedTargetIds = remainingClaimedTargets;
    } else {
