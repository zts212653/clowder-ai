...
export class CodexAgentService implements AgentService {
  ...
  private async *invokeInternal(
    ...
    // Collective participation supplies its own prompt; every other invocation
    // resolves session identity through the single hook pipeline (L0 retired).
    // fail-closed (generator contract, mirrors the CLI-not-found path below).
    const sessionPromptResult = participation
      ? { value: options!.systemPrompt! }
      : await this.prepareDeveloperInstructions(cliModel, options);
    ...
    const developerInstructions = participation
      ? sessionPromptResult.value
      : appendCatCafeGithubWriteRouting(sessionPromptResult.value, invocationApprovalSurface);
