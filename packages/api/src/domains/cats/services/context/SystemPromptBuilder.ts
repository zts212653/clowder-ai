...
- cat_cafe_create_thread: 创建新 thread（仅在 owner 显式请求时使用）。设置 parentThreadId 为当前 threadId 以便子 thread 知道向哪里汇报。创建后必须用 cross_post_message 将后续消息发到新 thread，不要在当前 thread 继续讨论新话题
