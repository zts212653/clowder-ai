...
export interface InterventionGateInput {
  ...
  /** F267 owner truth: fix/build/delete_sunset request authority; keep_observe never does. */
...
export const INTERVENTION_GATE_BLOCKER_TEXT: Record<InterventionGateBlockerCode, string> = {
  ...
  intervention_action_missing: 'measurement owner 没有发布本轮 action，不能把沉默解释成变更请求。',
  intervention_not_authority_requesting: 'owner 的结论是继续观察，不请求 change/adopt/continue-investment。',
};
...
export function evaluateInterventionGate(input: InterventionGateInput): InterventionGateVerdict {
  ...
  if (input.proposedAction === undefined) {
    blockers.push(blocker('intervention_action_missing'));
  } else if (input.proposedAction === 'keep_observe') {
    blockers.push(blocker('intervention_not_authority_requesting'));
  }
