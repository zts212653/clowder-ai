...
test('observes a newer catalog snapshot without rebuilding routes and never offers a downgrade', async () => {
  const alpha10 = {
    ...
    archiveUrl:
      'https://registry.npmjs.org/@clowder-ai/feishu-meeting-intake/-/feishu-meeting-intake-0.1.0-alpha.10.tgz',
    packageDigest: `sha512-${Buffer.alloc(64, 10).toString('base64')}`,
  };
  let current = entry;
  ...
  try {
    ...
    assert.equal(after.json().plugins[0].availableVersion, '0.1.0-alpha.10');
  ...
  const newerInstall = await harness({
    ...
    installedDigest: `sha512-${Buffer.alloc(64, 11).toString('base64')}`,
  });
  try {
    current = entry;
