...
describe('MountRulesStore (F228)', () => {
  ...
  test('validateMountRules rejects missing standard provider', () => {
    const broken = {
      version: 1,
      providers: { claude: { enabled: true, path: '.claude/skills' } },
      customPaths: [],
    };
  ...
  test('validateMountRules accepts legacy rules without customPaths field (F228 P2)', () => {
    ...
    const legacy = {
      version: 1,
      providers: DEFAULT_MOUNT_RULES.providers,
    };
    ...
    assert.deepEqual(result.providers, DEFAULT_MOUNT_RULES.providers, 'providers preserved');
  });
  ...
  test('writeMountRules persists to capabilities.json#mountRules for v2 projects', async () => {
    ...
    const back = await readMountRules(tempDir);
    assert.equal(back.providers.claude.enabled, true);
    assert.equal(back.providers.codex.enabled, false);
  ...
  test('readMountRules inherits defaultMountRules from main project when external project has no own mountRules', async () => {
    ...
    try {
      ...
      assert.equal(withoutMain.providers.codex.enabled, true, 'without main: codex should be default-enabled');

      // With mainProjectRoot: inherits defaultMountRules (codex disabled)
      const inherited = await readMountRules(tempDir, mainDir);
      assert.equal(inherited.providers.codex.enabled, false, 'with main: codex should inherit disabled');
      assert.equal(inherited.providers.claude.enabled, true, 'claude should inherit enabled');
      assert.equal(inherited.providers.gemini.enabled, true, 'gemini should inherit enabled');
    } finally {
  ...
  test('readMountRules applies defaultMountRules to main project when it has no own mountRules', async () => {
    ...
    try {
      ...
      assert.equal(
        result.providers.codex.enabled,
        false,
        'main project should use defaultMountRules when own rules absent',
      );
      assert.equal(result.providers.claude.enabled, true, 'main project should preserve enabled default providers');
    } finally {
      await rm(mainDir, { recursive: true, force: true });
    }
  });
  ...
  test('readMountRules uses project own mountRules before main defaultMountRules', async () => {
    ...
    try {
      ...
      assert.equal(result.providers.codex.enabled, true, 'project override should beat inherited default');
      assert.equal(result.providers.codex.path, '.project-codex/skills');
      assert.equal(result.providers.gemini.enabled, false, 'disabled provider should be preserved');
  ...
  test('readMountRules uses project own mountRules over main defaultMountRules', async () => {
    ...
    try {
      ...
      const result = await readMountRules(tempDir, mainDir);
      assert.equal(result.providers.codex.enabled, true, 'project override: codex should be enabled');
    } finally {
      await rm(mainDir, { recursive: true, force: true });
    }
  });
  ...
  test('readMountRules disables invalid standard mountRules entries from v2 config', async () => {
    ...
    const rules = await readMountRules(tempDir);
    assert.equal(rules.providers.codex.enabled, false, 'invalid standard rule must not fall back to enabled default');
