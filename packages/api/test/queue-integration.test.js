...
function mockRouter(opts = {}) {
  ...
  let lifecycleInvocationSequence = 0;
  ...
  return {
    ...
    router: {
      ...
      async *routeExecution(userId, message, threadId, userMessageId, targetCats, intent, options) {
        ...
        if (options.onLifecycleInvocationStarted) {
          lifecycleInvocationSequence += 1;
          await options.onLifecycleInvocationStarted({
            threadId,
            userId,
            catId: targetCats[0],
            invocationId: `integration-turn-${lifecycleInvocationSequence}`,
