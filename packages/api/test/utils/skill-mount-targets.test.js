...
import { buildProviderSkillDirCandidates, buildSkillMountTargets } from '../../dist/utils/skill-mount.js';
...
describe('buildSkillMountTargets (F228)', () => {
  ...
  test('standard HOME candidates stay canonical when project mount path is customized', () => {
    ...
    const candidates = buildProviderSkillDirCandidates(PROJECT, HOME, rules);
  ...
  test('omits disabled standard providers', () => {
    const rules = {
      ...DEFAULT_MOUNT_RULES,
      providers: {
        ...DEFAULT_MOUNT_RULES.providers,
  ...
  test('returns empty array when all standard providers disabled and no custom paths', () => {
    const rules = {
      version: 1,
      providers: {
