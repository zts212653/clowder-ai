...
const ROUTE_SOURCES = [
  ...
  new URL('../../src/routes/schedule-task-create-route.ts', import.meta.url),
];
...
describe('schedule mutation route structure', () => {
  it('keeps the route composition and extracted modules under the 350-line hard cap', () => {
    const routeSources = ROUTE_SOURCES.map((source) => readFileSync(source, 'utf8'));
    const [scheduleSource] = routeSources;
