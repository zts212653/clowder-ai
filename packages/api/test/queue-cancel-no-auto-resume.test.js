...
import { adaptInvocationQueue, adaptMessageStore, canonicalTestQueueInput } from './helpers/message-from-fixtures.js';
...
describe('cancelAll must NOT auto-resume queued entries', () => {
  ...
  it('multi-cat cancelAll does NOT leave stale suppress on secondary cats', async () => {
    ...
    assert.equal(requestDrain.mock.calls[0].arguments[0], 't1');
    assert.equal(requestDrain.mock.calls[0].arguments[1]?.bypassSuppressionForCatId, 'codex');
    assert.ok(
      Number.isInteger(requestDrain.mock.calls[0].arguments[1]?.bypassSuppressionEpoch),
      'the drain request must bind its bypass to the exact observed fence epoch',
    );
  });

  it('a drain bypass cannot cross a newer suppression epoch armed during target resolution', async () => {
    let admitResolution;
    let releaseResolution;
    const resolutionEntered = new Promise((resolve) => {
      admitResolution = resolve;
    });
    const resolutionGate = new Promise((resolve) => {
      releaseResolution = resolve;
    });
    deps.router.resolveExplicitTargets = mock.fn(async (requestedCatIds) => {
      admitResolution();
      await resolutionGate;
      return [...requestedCatIds];
    });
    enqueueEntry(deps.queue, { targetCats: ['codex'], content: 'must remain queued' });
    processor.suppressAutoResume('t1', 'codex', ['inv-old']);

    const completion = processor.onInvocationComplete('t1', 'codex', 'canceled_by_user', 'inv-other', []);
    await resolutionEntered;
    processor.suppressAutoResume('t1', 'codex', ['inv-newer']);
    releaseResolution();
    await completion;

    assert.equal(deps.router.routeExecution.mock.calls.length, 0);
    assert.equal(deps.queue.list('t1', 'u1').length, 1, 'the newer fence must keep the body queued');
  });
  ...
  it('single-cat user_cancel consumes the attempted primary and does not blind-spawn from cleanup', async () => {
    ...
    deps.messageStore = adaptMessageStore(new MessageStore());
    const source = deps.messageStore.append({
      threadId: 't1',
      userId: 'u1',
      catId: 'opus',
      content: 'hello',
      ...
      lifecycle: { kind: 'input', orderKey: '1:source-cancel', dispatchRefs: [] },
    });
