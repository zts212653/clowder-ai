  ...
  try {
    return execFileSync(
      ...
      [
        ...
        '-a',
        '-F',
        '#{pane_id} #{pane_pid} #{@cat-cafe-lease} #{pane_dead}',
      ],
    ...
    test(
      ...
      async () => {
        ...
        try {
          ...
          assert.match(
            created.state,
            new RegExp(`^${created.paneId} ${commandPid} [a-f0-9-]{36} 0$`, 'm'),
            'the command must really run with a stamped creation token before receipt damage',
          );
          assert.deepEqual(
            panes(gateway, wt),
            siblings,
