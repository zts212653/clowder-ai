...
function createHarness(overrides = {}) {
  ...
  const queueProcessor = {
    registerCallerDispatchQueueWithdrawal(carrier) {
      withdrawalObservations.push(carrier);
    },
...
describe('F264 Gap F true recall API', () => {
  ...
  it('atomically removes a zero-exposure carrier and returns the body only in the owner draft', async () => {
    ...
    assert.deepEqual(
      harness.withdrawalObservations.map((entry) => entry.id),
      entries.map((entry) => entry.id),
      'the successful Queue withdrawal must pass every committed carrier through the caller-view hook',
    );
