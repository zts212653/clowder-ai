...
describe('RFC #1356 Redis Queue ledger', { skip: redisIsolationSkipReason(REDIS_URL) }, () => {
  ...
  it('retires subsets from one shared multi-target claim', async () => {
    const entry = row('m-shared-claim', ['opus', 'codex']);
    await store.enqueue([entry]);
    const claimed = await store.claimPrefix('thread-redis', [entry.id], 'claim-shared', 200);
    assert.equal(claimed.outcome, 'claimed');
    assert.deepEqual(claimed.entries[0].claimedTargetIds, ['opus', 'codex']);

    const first = await store.commit('thread-redis', entry.id, 'claim-shared', 'processing', 201, undefined, ['opus']);
    assert.equal(first.outcome, 'updated');
    assert.deepEqual(first.entry.targets, ['opus']);
    const remaining = await store.get('thread-redis', entry.id);
    assert.equal(remaining.status, 'claimed');
    assert.equal(remaining.claimId, 'claim-shared');
    assert.deepEqual(remaining.targets, ['codex']);
    assert.deepEqual(remaining.claimedTargetIds, ['codex']);

    const second = await store.commit('thread-redis', entry.id, 'claim-shared', 'processing', 202, undefined, [
      'codex',
    ]);
    assert.equal(second.outcome, 'updated');
    assert.deepEqual(second.entry.targets, ['codex']);
