...
describe('resolveCatCarrier', () => {
  it('keeps legacy reads at one boundary and always returns the canonical vocabulary', () => {
    ...
    assert.equal(resolveCatCarrier({ clientId: 'openai', cli: { carrier: 'app_server' } }), 'app_server');
    assert.equal(resolveCatCarrier({ clientId: 'openai' }, 'app_server'), 'app_server');
    assert.equal(resolveCatCarrier({ clientId: 'openai' }, 'exec_json'), 'cli');
    assert.equal(resolveCatCarrier({ clientId: 'openai', acp: { command: 'opencode' } }, 'app_server'), 'acp');
    assert.equal(resolveCatCarrier({ clientId: 'anthropic', cli: { carrier: 'app_server' } }), 'cli');
