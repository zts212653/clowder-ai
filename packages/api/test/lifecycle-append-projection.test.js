...
function mixedTargetTracker() {
  const codexRun = {
    threadId: 'thread-1',
    targetId: 'codex',
    invocationId: 'turn-codex',
    responseMessageId: 'response-codex',
    inputEntryIds: ['old-entry'],
    inputMessageIds: ['old-message'],
    privateInputEntryIds: [],
    startedAt: 1,
  ...
  return {
    ...
    getUserId: () => 'user-1',
    cancel: () => ({ cancelled: false, catIds: [] }),
    getActiveSlots: () => [{ catId: 'codex', startedAt: 1, activeRun: codexRun }],
    getAgentClientActiveRunDispatcher: (_threadId, targetId) =>
      targetId === 'codex'
        ? {
            invocationId: 'turn-codex',
            capabilities: { append: true, steer: true },
...
describe('lifecycle Append action projection', () => {
  ...
  it('projects each available target independently when a sibling has no live Append carrier', () => {
    const result = projectLifecycleAppendAction({
      threadId: 'thread-1',
      userId: 'user-1',
      queueRevision: 'revision-1',
      entry: entry({ targets: ['codex', 'opus'] }),
      invocationTracker: mixedTargetTracker(),
    });
    ...
    assert.deepEqual(result, {
      ...
      action: {
        kind: 'append',
        expectedQueueRevision: 'revision-1',
        expectedRuns: [{ targetId: 'codex', invocationId: 'turn-codex', responseMessageId: 'response-codex' }],
      },
    });
  });
});
