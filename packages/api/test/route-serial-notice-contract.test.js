...
describe('route-serial notice contract', () => {
  it('keeps an inline action @handle as text without adding a public correction bubble', async () => {
    ...
    assert.equal(
      appendCalls.some((msg) => msg.content?.includes('Ready for @codex review')),
      true,
    );
    assert.equal(
