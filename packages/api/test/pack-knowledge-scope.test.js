...
describe('PackKnowledgeScope', () => {
  ...
  test('schema V6 migration adds pack_id column', async () => {
    ...
    assert.ok(CURRENT_SCHEMA_VERSION >= 6, `Current schema version should be >=6, got ${CURRENT_SCHEMA_VERSION}`);
    ...
    assert.equal(version.v, CURRENT_SCHEMA_VERSION, `Schema version should be ${CURRENT_SCHEMA_VERSION}`);
  });
});
