...
describe('Skills Route', () => {
  ...
  it('mountPaths containing a disabled provider does not inflate required count', async () => {
    ...
    try {
      ...
      const debugging = body.skills.find((skill) => skill.name === 'debugging');
      assert.ok(debugging, 'debugging skill should be present');
      assert.deepEqual(debugging.mountHealth.enabledProviders, ['claude', 'codex', 'gemini']);
  ...
  it('GET /api/skills ignores same-id plugin capabilities for Cat Cafe source skill policy', async () => {
    ...
    try {
      ...
      const body = JSON.parse(res.body);
      const debugging = body.skills.find((skill) => skill.name === 'debugging');
      assert.ok(debugging, 'debugging Cat Cafe source skill should still be listed');
      assert.equal(debugging.source, 'cat-cafe');
      assert.equal(debugging.globalEnabled, true, 'same-id plugin disable must not disable Cat Cafe skill');
      assert.deepEqual(debugging.mountPaths, [], 'unmounted Cat Cafe skill should report actual mounted providers');
      assert.deepEqual(debugging.mountHealth, {
        enabledProviders: ['claude', 'codex', 'gemini', 'kimi'],
        mountedCount: 0,
        requiredCount: 4,
