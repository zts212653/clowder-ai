...
export const useMarketplaceStore = create<MarketplaceState>((set, get) => ({
  ...
  search: async (q: string) => {
    ...
    try {
      ...
      const data = (await res.json()) as { results: MarketplaceSearchResult[]; sources?: MarketplaceSourceStatus[] };
      set({ results: data.results ?? [], sources: data.sources ?? [], loading: false });
    } catch (err) {
