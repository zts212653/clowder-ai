...
export function useSkillControls() {
  ...
  const handleToggle = useCallback(
    async (
      ...
      try {
        ...
        // providers mounted, conflicting ones skipped for user resolution).
        const data = (await res.json().catch(() => ({}))) as {
          propagationConflicts?: { projectPath: string; provider: string }[];
        };
        ...
        if (conflictCount > 0) {
          setError(
            `全局启用成功，但 ${conflictCount} 个挂载冲突已跳过（目标路径已有用户自定义内容，可在 Skill 同步中处理）`,
          );
        }
      } catch {
        setError('网络错误');
  ...
  // F228: per-provider mount toggle (scope from caller + providerId)
  const handleProviderToggle = useCallback(
    async (
      ...
      setError(null);
      setToggling(`${skillId}:${providerId}`);
      try {
        const projectContext = toggleScope === 'project' ? (projectPathRef.current ?? undefined) : undefined;
        const body: Record<string, unknown> = {
          capabilityId: skillId,
