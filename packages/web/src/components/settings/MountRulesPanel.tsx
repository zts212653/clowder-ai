...
interface MountRules {
  version: 1;
  providers: Record<StandardId, StandardRule>;
...
export function MountRulesPanel({ projectPath, scope = 'project', onSaved }: MountRulesPanelProps) {
  ...
  const toggleStandard = useCallback(
    (id: StandardId) => {
      ...
      void saveRules({
        ...
        providers: {
          ...rules.providers,
          [id]: { ...rules.providers[id], enabled: !rules.providers[id].enabled },
        },
      });
    },
    [rules, saveRules],
  ...
  const summary = rules
    ? `${STANDARD_IDS.filter((id) => rules.providers[id].enabled).length}/4 标准 + ${rules.customPaths.length} 自定义`
    : '点击展开';
    ...
    <div className="rounded-2xl bg-[var(--console-card-bg)] shadow-[0_12px_30px_rgba(43,33,26,0.08)]">
      ...
      {open && (
        <div className="px-4 pb-3">
          ...
          {rules && !loading && (
            <>
              <p className="mb-2 text-xs font-semibold text-cafe-secondary">标准 Provider</p>
              <div className="space-y-1.5">
                {STANDARD_IDS.map((id) => (
                  <label key={id} className="flex items-center gap-3 text-sm">
                    <input
                      type="checkbox"
                      checked={rules.providers[id].enabled}
                      disabled={saving}
                      onChange={() => toggleStandard(id)}
                      className="h-4 w-4"
                    />
                    <span className="font-medium text-cafe">{id}</span>
                    <code className="text-xs text-cafe-muted">{rules.providers[id].path}</code>
                  </label>
                ))}
              </div>
