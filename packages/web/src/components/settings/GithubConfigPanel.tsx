...
export function GithubConfigPanel() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (updates.length === 0) { setResult({ type: 'error', message: '请填写至少一个配置项' }); return; }
        ...
        <div className="space-y-2">
          {fields.map((field) => (
            <div key={field.envName}>
              <label htmlFor={`plugin-config-${field.envName}`} className="mb-1 block text-xs font-medium text-cafe-secondary">
                {field.label}
              </label>
              <input
                id={`plugin-config-${field.envName}`}
                type={field.sensitive ? 'password' : 'text'}
                placeholder={field.sensitive ? (field.currentValue ? '已设置（输入新值覆盖）' : '未配置') : (field.currentValue ?? '未配置')}
          ...
          {result && (
            <div className={`rounded-[16px] px-3 py-2 text-xs ${
              result.type === 'success'
