...
describe('deriveActiveCats', () => {
  ...
  it('filters out roster cats already marked done when no live slot remains', () => {
    const result = deriveActiveCats(['opus', 'codex'], {}, { opus: 'done', codex: 'streaming' });
    expect(result).toEqual(['codex']);
  });

  it('keeps a done cat visible while its invocation slot still exists', () => {
    const result = deriveActiveCats(
      ['opus', 'codex'],
      {
        'inv-1': { catId: 'opus', mode: 'ideate' },
      },
      { opus: 'done', codex: 'streaming' },
    );
    expect(result).toEqual(['codex', 'opus']);
  });
