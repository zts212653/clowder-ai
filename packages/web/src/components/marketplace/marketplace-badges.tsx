...
const ECOSYSTEM_STYLES: Record<MarketplaceEcosystem, { bg: string; text: string; label: string }> = {
  ...
  openclaw: { bg: 'bg-[var(--semantic-error-bg)]', text: 'text-[var(--semantic-error-text)] border-[var(--semantic-error-text)]/20', label: 'OpenClaw' },
...
const TRUST_STYLES: Record<TrustLevel, { bg: string; text: string; label: string; iconName: string }> = {
  official: { bg: 'bg-[var(--semantic-success-bg)]', text: 'text-[var(--semantic-success-text)]', label: 'official', iconName: 'shield' },
  verified: { bg: 'bg-[var(--semantic-success-bg)]', text: 'text-[var(--semantic-success-text)]', label: 'verified', iconName: 'check' },
  community: { bg: 'bg-[var(--semantic-info-bg)]', text: 'text-[var(--semantic-info-text)]', label: 'community', iconName: 'users' },
};
...
const MODE_STYLES: Record<InstallMode, { bg: string; text: string; label: string; iconName: string }> = {
  direct_mcp: { bg: 'bg-[var(--semantic-success-bg)]', text: 'text-[var(--semantic-success-text)]', label: '一键安装', iconName: 'zap' },
  delegated_cli: { bg: 'bg-[var(--semantic-info-bg)]', text: 'text-[var(--semantic-info-text)]', label: 'CLI 安装', iconName: 'terminal' },
  manual_file: { bg: 'bg-[var(--semantic-warning-bg)]', text: 'text-[var(--semantic-warning-text)]', label: '手动配置', iconName: 'file-text' },
  manual_ui: { bg: 'bg-[var(--semantic-warning-bg)]', text: 'text-[var(--semantic-warning-text)]', label: '手动配置', iconName: 'file-text' },
};
