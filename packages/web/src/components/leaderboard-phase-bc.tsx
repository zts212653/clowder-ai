    ...
    <ul className="space-y-2">
      {entries.slice(0, 5).map((e) => (
        <li key={e.catId} className="flex items-center gap-2">
          ...
          <span className="text-xs ml-auto font-medium" style={{ color: 'var(--hub-lb-accent-warm)' }}>
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="flex flex-col gap-1">
        <span className="text-3xl font-medium" style={{ fontFamily: 'Fraunces, serif', color: 'var(--hub-lb-accent)' }}>
          {stats.catKill.wins}
        </span>
        <span className="text-xs" style={{ color: 'var(--hub-lb-text-muted)' }}>
          猫猫杀 胜场
        </span>
        {stats.catKill.topCat && (
          <span className="text-xs font-semibold" style={{ color: 'var(--hub-lb-accent-warm)' }}>
    ...
    <div className="flex flex-wrap gap-3">
      {achievements.map((a) => (
        <div
          ...
          style={{ background: 'var(--hub-lb-badge-bg)' }}
          title={a.description}
        >
          ...
          <span className="text-xs font-semibold" style={{ color: 'var(--hub-lb-accent)' }}>
            {a.label}
          </span>
        </div>
      ))}
...
export function CvoLevelCard({ level }: { level: CvoLevel }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs" style={{ color: 'var(--hub-lb-text-muted)' }}>
        {level.description}
      </p>
      <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--hub-lb-progress-bg)' }}>
        <div className="h-full rounded-full" style={{ width: `${pct}%`, background: 'var(--hub-lb-accent)' }} />
      </div>
      {level.nextTitle && (
        <span className="text-xs" style={{ color: 'var(--hub-lb-text-muted)' }}>
