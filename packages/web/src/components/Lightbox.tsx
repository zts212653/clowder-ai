...
export function CopyButton({ url }: { url: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 text-[var(--cafe-surface)] rounded-md px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity"
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute -top-3 -right-3 bg-black/70 hover:bg-black/90 text-[var(--cafe-surface)] rounded-full w-8 h-8 flex items-center justify-center text-lg z-10"
          title="Close"
        >
          &times;
        </button>
